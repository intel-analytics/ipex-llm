...
object SpatialBatchNormalization {
  ...
  private[bigdl] def updateOutputNHWCInferFloat(input: Tensor[Float], output: Tensor[Float],
    ...
    Log4Error.invalidInputError(input.isContiguous(), "BatchNorm NHWC Log4Error.invalidInputError a contiguous input")
  ...
  private[bigdl] def updateOutputNCHWInferFloat(input: Tensor[Float], output: Tensor[Float],
    ...
    Log4Error.invalidInputError(input.isContiguous(), "BatchNorm NCHW Log4Error.invalidInputError a contiguous input")
    val inputData = input.storage().array()
    val inputOffset = input.storageOffset() - 1
    val outputData = output.storage().array()
    val outputOffset = output.storageOffset() - 1
  ...
  private[bigdl] def updateGradInputNHWCTrainFloat(
    ...
    Log4Error.invalidInputError(input.nDimension() == 4, "BN Log4Error.invalidInputError a 4D input")
    Log4Error.invalidInputError(input.isContiguous(), "input is not contiguous")
    Log4Error.invalidInputError(gradOutput.nDimension() == 4, "BN Log4Error.invalidInputError a 4D gradient")
    Log4Error.invalidInputError(gradOutput.isContiguous(), "gradient is not contiguous")
    val nChannel = gradOutput.size(4)
    Log4Error.invalidInputError(saveMean.size(1) == nChannel, "saveMean length is not consistent with channel number")
    Log4Error.invalidInputError(saveStd.size(1) == nChannel, "saveStd length is not consistent with channel number")
    ...
    if (scale != null) {
      Log4Error.invalidInputError(scale.size(1) == nChannel, "scale length is not consistent with channel number")
