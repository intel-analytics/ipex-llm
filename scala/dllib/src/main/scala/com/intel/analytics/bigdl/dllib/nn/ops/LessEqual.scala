...
class LessEqual[T: ClassTag]()
  ...
  override def compareBoolean(a: Boolean, b: Boolean): Boolean = {
    Log4Error.invalidOperationError(false, "Does not support LessEqual on Boolean")
    true
  }

  override def compareByteString(a: ByteString, b: ByteString): Boolean = {
    Log4Error.invalidOperationError(false, "Does not support LessEqual on ByteString")
    true
  }
}
