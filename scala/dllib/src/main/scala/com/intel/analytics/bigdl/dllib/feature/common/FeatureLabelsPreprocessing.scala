 ...
 * @param labelStep Preprocessing for label, transform L to Table
 ...
 */
class FeatureLabelsPreprocessing[F, X, L, T: ClassTag] private[bigdl] (
    featureStep: Preprocessing[F, X],
    labelStep: Preprocessing[L, Array[Tensor[T]]]
...
object FeatureLabelsPreprocessing {
  def apply[F, X, L, T: ClassTag](
      featureStep: Preprocessing[F, X],
      labelStep: Preprocessing[L, Array[Tensor[T]]]
    )(implicit ev: TensorNumeric[T]): FeatureLabelsPreprocessing[F, X, L, T] =
    new FeatureLabelsPreprocessing(featureStep, labelStep)
}
