...
class SpatialDropout3DSpec extends TorchSpec {
  "SpatialDropout3D module with continuous input" should "converge to correct weight and bias" in {
    torchCheck()
    val module = SpatialDropout3D[Double](0.7)
    val input = Tensor[Double](3, 4, 5, 6, 7)
    val seed = 100
