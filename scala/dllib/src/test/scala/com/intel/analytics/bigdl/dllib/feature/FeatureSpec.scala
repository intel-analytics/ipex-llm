...
class FeatureSpec extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  "BigDLAdapter" should "adapt BigDL Transformer" in {
    ...
    TestUtils.conditionFailTest(newResize.isInstanceOf[Preprocessing[_, _]])
  }
  ...
  "Local ImageSet" should "work with resize" in {
    ...
    TestUtils.conditionFailTest(imf.getHeight() == 200)
    TestUtils.conditionFailTest(imf.getWidth() == 200)
    TestUtils.conditionFailTest(imf.getChannel() == 3)

    val imageGray = ImageSet.read(gray.getFile, resizeH = 200, resizeW = 200)
    val imfGray = imageGray.toLocal().array.head
    TestUtils.conditionFailTest(imfGray.getHeight() == 200)
    TestUtils.conditionFailTest(imfGray.getWidth() == 200)
    TestUtils.conditionFailTest(imfGray.getChannel() == 1)
  }
  ...
  "ImageBytesToMat" should "work with png and jpg" in {
    ...
    val jpg = image -> ImageBytesToMat(imageCodec = Imgcodecs.IMREAD_UNCHANGED)
    ...
    TestUtils.conditionFailTest(height == height2 && width == width2)
    TestUtils.conditionFailTest(channel == 3 && channel2 == 4)
  }
  ...
  "ImageNormalize" should "work with min max normType" in {
    ...
    val imfJpg = jpg.toDistributed().rdd.collect().head
    imfJpg.floats().foreach{ t =>
      TestUtils.conditionFailTest(t>=0 && t<=1.0)}
  }
  ...
  "ImageMatToTensor" should "work with both NCHW and NHWC" in {
    ...
    TestUtils.conditionFailTest(nhwc.isContiguous() == true)

    val data2 = ImageSet.read(resource.getFile)
    TestUtils.conditionFailTest(
      data2.toLocal().array.head.apply[Tensor[Float]](ImageFeature.imageTensor) == null)
