...
class GlobalMaxPooling3DSpec extends KerasBaseSpec{
  ...
  "GlobalMaxPooling3D" should "be the same as Keras" in {
    val kerasCode =
      """
        |input_tensor = Input(shape=[3, 4, 5, 6])
        |input = np.random.random([2, 3, 4, 5, 6])
        |output_tensor = GlobalMaxPooling3D(dim_ordering="th")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = KSequential[Float]()
    val layer = GlobalMaxPooling3D[Float](inputShape = Shape(3, 4, 5, 6))
    seq.add(layer)
...
class GlobalMaxPooling3DSerialTest extends ModuleSerializationTest {
  override def test(): Unit = {
    ...
    val input = Tensor[Float](2, 12, 24, 3, 6).apply1(_ => new SecureRandom().nextFloat())
