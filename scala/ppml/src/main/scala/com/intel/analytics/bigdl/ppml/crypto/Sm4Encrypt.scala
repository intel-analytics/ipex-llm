...
class Sm4Encrypt extends Crypto {
  ...
  def encryptFile(binaryFilePath: String, savePath: String, dataKeyPlaintext: String) = {
    ...
    val encryptedBytes = timing("SM4Cryptos encrypting a single file") {
      encryptContent(content, dataKeyPlaintext)
    }
    timing("SM4Cryptos save a encrypted file") {
  ...
  def decryptFile(binaryFilePath: String, savePath: String, dataKeyPlaintext: String) = {
    ...
    val decryptedBytes = timing("SM4Cryptos decrypt a single file...") {
      decryptContent(content, dataKeyPlaintext)
    }
    timing("SM4Cryptos save a decrypted file") {
  ...
  private def encryptContent(content: Array[Byte], dataKeyPlaintext: String): Array[Byte] = {
    ...
    val encryptionKeySpec: SecretKeySpec = new SecretKeySpec(encryptKey, "SM4")

    val cipher: Cipher = Cipher.getInstance(CryptoMode.SM4_ECB_PKCS5PADDING.value, BouncyCastleProvider.PROVIDER_NAME)
  ...
  private def decryptContent(content: Array[Byte], dataKeyPlaintext: String): Array[Byte] = {
    ...
    val secretKeySpec = new SecretKeySpec(encryptKey, "SM4")
