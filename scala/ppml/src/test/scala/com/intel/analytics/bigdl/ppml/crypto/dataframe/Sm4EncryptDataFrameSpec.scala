...
import com.intel.analytics.bigdl.ppml.crypto.{CryptoMode, Sm4Encrypt}
...
class Sm4EncryptDataFrameSpec extends FlatSpec with Matchers with BeforeAndAfter{
  ...
  def generateCsvData(): (String, String, String) = {
    ...
    val encryptFileName = dir + "/sm4_en_people.csv"
    ...
    val encryptedBytes = sm4Cryptos.encryptBytes(data.toString().getBytes, dataKeyPlaintext)
  ...
  "textfile read from plaint text file" should "work" in {
    ...
    file.mkString("\n") should be (data)
    val file2 = sc.textFile(encryptFileName, cryptoMode = CryptoMode.SM4_ECB_PKCS5PADDING).collect()
    file2.mkString("\n") should be (data)
  }
  ...
  "read from encrypted csv with header" should "work" in {
    val df = sc.read(cryptoMode = CryptoMode.SM4_ECB_PKCS5PADDING)
  ...
  "read from encrypted csv without header" should "work" in {
    val df = sc.read(cryptoMode = CryptoMode.SM4_ECB_PKCS5PADDING).csv(encryptFileName)
    val d = df.collect().map(v => s"${v.get(0)},${v.get(1)},${v.get(2)}").mkString("\n")
    d should be (data)
  }
