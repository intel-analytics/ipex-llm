...
class Threshold[@specialized(Float, Double) T: ClassTag](th: Double = 1e-6, v: Double = 0.0, ip: Boolean = false)(implicit ev: TensorNumeric[T]) extends Module[T] {
  var threshold = th
  var value = v
  var inPlace = ip
  validateParameters()
  ...
  override  def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    val taskSize = input.nElement() / coresNum
    var extraTaskSize = input.nElement() % coresNum
    var allocated = 0
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              ...
              while (i < taskArray(_t)._2) {
                inputData(inputOffset + i) = if (inputData(inputOffset + i) <= threshold) value else inputData(inputOffset + i)
                i += 1
              }
            }
            t += 1
        ...
        case "Float" => {
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              var i = taskArray(_t)._1
              while (i < taskArray(_t)._2) {
                inputData(inputOffset + i) = if (inputData(inputOffset + i) <= threshold) valueFloat else inputData(inputOffset + i)
                i += 1
              }
            }
            t += 1
        ...
        case "Double" => {
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              var i = taskArray(_t)._1
              while (i < taskArray(_t)._2) {
                outputData(outputOffset + i) = if (inputData(inputOffset + i) > threshold) inputData(inputOffset + i) else value
                i += 1
              }
            }
            t += 1
        ...
        case "Float" => {
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              var i = taskArray(_t)._1
              while (i < taskArray(_t)._2) {
                outputData(outputOffset + i) = if (inputData(inputOffset + i) > threshold) inputData(inputOffset + i) else valueFloat
                i += 1
              }
            }
            t += 1
  ...
  private def updateGradInputNoContinuous(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    if(inPlace)
    {
      ...
      ev.getType() match {
        case "Double" => gradInput.asInstanceOf[Tensor[Double]].map(input.asInstanceOf[Tensor[Double]],(g,i)=>
          if(i<=threshold) 0 else g)
        case "Float" => gradInput.asInstanceOf[Tensor[Float]].map(input.asInstanceOf[Tensor[Float]],(g,i)=>
          if(i<=threshold) 0 else g)
        case _ => throw new UnsupportedOperationException(s"Only Float/Double supported")
      }
    }
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    val taskSize = gradOutput.nElement() / coresNum
    var extraTaskSize = gradOutput.nElement() % coresNum
    var allocated = 0
    val tasks = new ArrayBuffer[(Int, Int)]()
        ...
        case "Double" =>
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              var i = taskArray(_t)._1
              while (i < taskArray(_t)._2) {
                gradInputData(gradInputOffset + i) = if (inputData(inputOffset + i) <= threshold) 0.0 else gradInputData(gradInputOffset + i)
                i += 1
              }
            }
            t += 1
        ...
        case "Float" =>
          ...
          while(t < taskArray.length) {
            ...
            results(_t) = Future {
              var i = taskArray(_t)._1
              while (i < taskArray(_t)._2) {
                gradInputData(gradInputOffset + i) = if (inputData(inputOffset + i) <= threshold) 0.0f else gradInputData(gradInputOffset + i)
                i += 1
              }
            }
            t += 1
  ...
  def validateParameters()={
    if(inPlace) require(value <= threshold,"in-place processing requires value (" + value + "') not exceed threshold ("  +threshold + ")")
  }
