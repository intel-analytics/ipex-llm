...
object File {
   ...
   * @param objectType
   */
  def save(source : Any,fileName : String, objectType : TorchObject) = {
    val capacity = 300
    val path = Paths.get(fileName)
  ...
  private def writeObject(source : Any, rawdata : ByteBuffer, path : Path, objectType : TorchObject): Unit ={
    flush(rawdata, path)
  ...
  private def writeVersionAndClass(version : String, className : String, rawdata : ByteBuffer, path : Path): Unit ={
  ...
  private def writeSpatialConvolution(source : SpatialConvolution[Double], rawdata : ByteBuffer,path : Path): Unit ={
  ...
  private def writeSpatialMaxPooling(source : SpatialMaxPooling[Double], rawdata : ByteBuffer,path : Path): Unit ={
    var table : Map[String, Any] = new HashMap()
  ...
  private def writeSequential(source : Sequential[Double], rawdata : ByteBuffer,path : Path): Unit ={
    var table : Map[String, Any] = new HashMap()
    val output = source.output
    val gradInput = source.gradInput
    val modules : Map[Double, Module[Double]] = new HashMap()
  ...
  private def writeTable(source : Map[Any,Any], rawdata : ByteBuffer,path : Path): Unit ={
    ...
    while(it.hasNext()) {
      ...
      else if (source.get(key).isInstanceOf[Tensor[Double]]) {
      ...
      else if (source.get(key).isInstanceOf[Map[Any,Any]]){
        writeObject(source.get(key).asInstanceOf[Map[Any,Any]], rawdata, path, TYPE_TABLE)
      }
      else if (source.get(key).isInstanceOf[Linear[Double]]){
  ...
  // Modules
  private def readSpatialMaxPooling(rawData : ByteBuffer, objects : Map[Int, Any]) : SpatialMaxPooling[Double] = {
  ...
  private def readSpatialAveragePooling(rawData : ByteBuffer, objects : Map[Int, Any]) : SpatialAveragePooling[Double] = {
    val elements = readObject(rawData, objects).asInstanceOf[Map[String, Any]]
    val padW = elements.get("padW").asInstanceOf[Double].toInt
    val padH = elements.get("padH").asInstanceOf[Double].toInt
    val dW = elements.get("dW").asInstanceOf[Double].toInt
    ...
    val result = new SpatialAveragePooling[Double](kW, kH, dW, dH, padW, padH, ceilMode, countIncludePad, divide)
    val output = elements.get("output").asInstanceOf[Tensor[Double]]
    val gradInput = elements.get("gradInput").asInstanceOf[Tensor[Double]]
    result.output.resizeAs(output)
    result.output.copy(output)
  ...
  private def readConcat(rawData : ByteBuffer, objects : Map[Int, Any]) : Concat[Double] = {
    ...
    val output = elements.get("output").asInstanceOf[Tensor[Double]]
    val gradInput = elements.get("gradInput").asInstanceOf[Tensor[Double]]
    val size = elements.get("size").asInstanceOf[Array[Int]] // size array will be adjust to the input in the training
  ...
  private def readDropout(rawData : ByteBuffer, objects : Map[Int, Any]) : Dropout[Double] = {
    ...
    //val v2 = elements.get("v2").asInstanceOf[Boolean]
  ...
  private def readSpatialConvolutionMap(rawData : ByteBuffer, objects : Map[Int, Any]) : SpatialConvolutionMap[Double] = {
...
//    result.fGradInput.copy(fgradInput)
  ...
  private def readBatchNormalization(rawData : ByteBuffer, objects : Map[Int, Any]) : BatchNormalization[Double] = {
    val elements = readObject(rawData, objects).asInstanceOf[Map[String, Any]]
  ...
  private def readSpatialBatchNormalization(rawData : ByteBuffer, objects : Map[Int, Any]) : SpatialBatchNormalization[Double] = {
    val elements = readObject(rawData, objects).asInstanceOf[Map[String, Any]]
    val eps = elements.get("eps").asInstanceOf[Double]
    val momentum = elements.get("momentum").asInstanceOf[Double]
    val affine = elements.get("affine").asInstanceOf[Boolean]
  ...
  private def readSpatialZeroPadding(rawData : ByteBuffer, objects : Map[Int, Any]) : SpatialZeroPadding[Double] = {
    val elements = readObject(rawData, objects).asInstanceOf[Map[String, Any]]
  ...
  private def readSpatialConvolution(rawData : ByteBuffer, objects : Map[Int, Any]): SpatialConvolution[Double] = {
    val elements = readObject(rawData, objects).asInstanceOf[Map[String, Any]]
    val padH = elements.get("padH").asInstanceOf[Double].toInt
    val padW = elements.get("padW").asInstanceOf[Double].toInt
    val dH = elements.get("dH").asInstanceOf[Double].toInt
    ...
    val gradWeight = elements.get("gradWeight").asInstanceOf[Tensor[Double]]
    val fgradInput = elements.get("fgradInput").asInstanceOf[Tensor[Double]]
    val result = new SpatialConvolution[Double](nInputPlane,nOutputPlane,kW,kH,dW,dH,padW,padH)
    result.gradBias.resizeAs(gradBias)
    result.gradBias.copy(gradBias)
    result.weight.resizeAs(weight)
    result.weight.copy(weight)
  ...
  private def readSequentialModule(rawData : ByteBuffer, objects : Map[Int, Any]) : Sequential[Double] = {
