 ...
 */
class LBFGS[@specialized(Float, Double) T:ClassTag](implicit ev: TensorNumeric[T]) extends OptimMethod[T] with IterateByItself {
  private var verbose : Boolean = false

  /**
   * Optimize the model parameter
   *
   * @param opfunc a function that takes a single input (X), the point of a evaluation, and returns f(X) and df/dX
   ...
   */
  override def optimize(opfunc: (Tensor[T]) => (T, Tensor[T]), x: Tensor[T], config: Table, state: Table): (Tensor[T], Array[T]) = {
