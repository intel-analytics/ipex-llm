  ...
  override def updateGradInput(input: Tensor[Float], gradOutput: Tensor[Float]): Tensor[Float] = {
    ...
    if (update_primitive) {
      ...
      val bwd_desc = MklDnnOps.poolingBackwardDescInit(algKind, gradInput_md,
                                                  gradOutput_md, strides, kernel, paddingTL,
                                                  paddingBR, MklDnn.PaddingKind.mkldnnPaddingZero)
    ...
    val (memoryPrimitives, buffer) =
      if (reorder_gradOutput_memory == 0L) {
        ...
        (Array(gradOutput_memory, reorder_gradOutput_memory, gradInput_memory),
        Array(gradOutput, gradOutputBuffer, gradInput))
      }
