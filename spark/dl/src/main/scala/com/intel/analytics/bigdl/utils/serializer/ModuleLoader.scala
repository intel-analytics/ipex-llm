...
object ModulePersister {
  ...
  private def validateModuleNames[T: ClassTag]
    (module : AbstractModule[Activity, Activity, T]): Unit = {
    val names = new mutable.HashSet[String]()
    validateModuleNames(module, names)
  }

  private def validateModuleNames[T: ClassTag]
  (module : AbstractModule[Activity, Activity, T], names: mutable.HashSet[String]): Unit = {
    module match {
      case container : Container[_, _, _] =>
        container.modules.foreach(module => validateModuleNames(module, names))
      case _ =>
        require(!names.contains(module.getName), "Module name should be unique!")
