...
import com.intel.analytics.bigdl.dlframes.{DLClassifierModel, DLImageReader, DLImageTransformer, DLModel}
...
object ImageInference {
  ...
  def main(args: Array[String]): Unit = {
    ...
    Utils.parser.parse(args, defaultParams).foreach { params =>
      val conf = Engine.createSparkConf().setAppName("DLFramesInference")
      val sc = SparkContext.getOrCreate(conf)
      val sqlContext = new SQLContext(sc)
      Engine.init

      val imageDF = DLImageReader.readImages(params.folder, sqlContext.sparkContext)
      val transformer = Resize(256, 256) -> CenterCrop(224, 224) ->
        ChannelNormalize(123, 117, 104) -> MatToTensor() -> ImageFrameToSample()
      val featureDF = new DLImageTransformer(transformer).transform(imageDF)
      ...
      resultDF.select("imageName", "prediction").show(100, false)
    }
  }
}
