...
object TensorflowToBigDL {
  ...
  private var patternList : ArrayBuffer[TensorflowToBigDL] = {
    ...
    res.append(
      ...
      Conv2DWithoutBias, NotEqualTF,
      ExpandDimsTF, MergeTF, SwitchTF, DecodeRawTF, LogicalAndTF, LogicalOrTF,
      DecodeGifTF, DecodeJpegTF, DecodePngTF, NoOpTF, NotEqualTF, LessTF, LogicalNotTF, SubStrTF,
      SumTF, CastTF
    )
    res
  }
...
object ExpandDimsTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("ExpandDims")
    Node("*") -> node
    Node("Const") -> node
  ...
  override def layer[T: ClassTag](tfGraph: DirectedGraph[NodeDef],
    ...
    val node = tfGraph.source.prevNodes(1)

    val dim = TFUtils.parseTensor(node.element.getAttrMap.get("value").getTensor, byteOrder)
      .asInstanceOf[Tensor[Int]].value() + 1
...
object MergeTF extends TensorflowToBigDL {
...
object SwitchTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("Switch")
    Node("*") -> node
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object DecodeRawTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("DecodeRaw")
    Node("*") -> node
    node.graph(reverse = true)
  }
  ...
  override def layer[T: ClassTag](tfGraph: DirectedGraph[NodeDef],
    ...
    val attrs = tfGraph.source.element.getAttrMap

    val endian = getBoolean(attrs, "little_endian")

    val outType = attrs.get("out_type").getType

    new DecodeRaw[T](outType, endian)
      .asInstanceOf[AbstractModule[Activity, Activity, T]]
  }
}

object LogicalOrTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("LogicalOr")
    Node("*") -> node
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object LogicalAndTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("LogicalAnd")
    Node("*") -> node
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object LogicalNotTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("LogicalNot")
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object DecodeJpegTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("DecodeJpeg")
    Node("*") -> node
    node.graph(reverse = true)
  }
  ...
  override def layer[T: ClassTag](tfGraph: DirectedGraph[NodeDef],
    ...
    val ratio = getInt(attr, "ratio")

    new DecodeJpeg(channels, ratio)
      .asInstanceOf[AbstractModule[Activity, Activity, T]]
  }
}

object DecodePngTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("DecodePng")
    Node("*") -> node
    node.graph(reverse = true)
  }
  ...
  override def layer[T: ClassTag](tfGraph: DirectedGraph[NodeDef],
    ...
    new DecodePng(channels)
      .asInstanceOf[AbstractModule[Activity, Activity, T]]
  }
}

object DecodeGifTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("DecodeGif")
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object NoOpTF extends TensorflowToBigDL {
...
object NotEqualTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("NotEqual")
    Node("*") -> node
    Node("*") -> node
    node.graph(reverse = true)
  }
...
object LessTF extends TensorflowToBigDL {
...
object SubStrTF extends TensorflowToBigDL {

  private val graph = {
    val node = Node("Substr")
    Node("*") -> node
    Node("*") -> node
    Node("*") -> node
    node.graph(reverse = true)
...
object SumTF extends TensorflowToBigDL {
  ...
  override def layer[T: ClassTag](tfGraph: DirectedGraph[NodeDef],
   ...
   implicit ev: TensorNumeric[T]): AbstractModule[Activity, Activity, T] = {
    ...
    val const = tfGraph.source.prevNodes(1)
    val attr = tfGraph.source.element.getAttrMap

    val squeeze = !getBoolean(attr, "keep_dims")

    val dims = TFUtils.parseTensor(const.element.getAttrMap.get("value").getTensor,
      ByteOrder.LITTLE_ENDIAN).asInstanceOf[Tensor[Int]]
    ...
    val dataType = getType(attr, "T")
    dataType match {
      case DataType.DT_INT8 => dim.foreach(i => sum.add(new Sum[T, Int](i, squeeze = squeeze)))
      case DataType.DT_INT16 => dim.foreach(i => sum.add(new Sum[T, Int](i, squeeze = squeeze)))
      case DataType.DT_UINT8 => dim.foreach(i => sum.add(new Sum[T, Int](i, squeeze = squeeze)))
      case DataType.DT_UINT16 => dim.foreach(i => sum.add(new Sum[T, Int](i, squeeze = squeeze)))
      case DataType.DT_INT32 => dim.foreach(i => sum.add(new Sum[T, Int](i, squeeze = squeeze)))
      case DataType.DT_INT64 => dim.foreach(i => sum.add(new Sum[T, Long](i, squeeze = squeeze)))
      case DataType.DT_FLOAT => dim.foreach(i => sum.add(new Sum[T, Float](i, squeeze = squeeze)))
      case DataType.DT_DOUBLE => dim.foreach(i => sum.add(new Sum[T, Double](i, squeeze = squeeze)))
    }

    sum.asInstanceOf[AbstractModule[Activity, Activity, T]]
  }
}

object CastTF extends TensorflowToBigDL {
