...
/**
 * Convert bytes to resized BGR IMG
 * @param resizeWidth IMG width after resize
 * @param resizeHeight IMG height after resize
 */
class BytesToResizedBGRImg(resizeWidth : Int, resizeHeight : Int)
  ...
  override def apply(prev: Iterator[ByteRecord]): Iterator[LabeledBGRImage] = {
    prev.map(record => {
      ...
      buffer.copy(resizeImage(bufferedImage, resizeWidth, resizeHeight),
        1.0f).setLabel(record.label)
    })
  }
...
object BytesToResizedBGRImg {
  def apply(resizeWidth: Int, resizeHeight: Int): BytesToResizedBGRImg =
    new BytesToResizedBGRImg(resizeWidth, resizeHeight)
}
