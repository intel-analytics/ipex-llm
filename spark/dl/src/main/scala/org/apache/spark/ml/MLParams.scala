...
import org.apache.spark.ml.OptimMethodParam._
...
class OptimMethodParam(
    ...
    name: String,
    doc: String,
    isValid: OptimMethod[_] => Boolean)
  extends Param[OptimMethod[_]](parent, name, doc, isValid) {

  def this(parent: Params, name: String, doc: String) =
    this(parent, name, doc, (_: OptimMethod[_]) => true)
...
object AdagradBuilder extends OptimMethodBuilder {
  ...
  override def decode[T: ClassTag](json: JObject)(implicit ev: TensorNumeric[T]): Adagrad[T] = {
    ...
    val adagrad = if (ev.getType() == DoubleType) {
    ...
    adagrad.asInstanceOf[Adagrad[T]]
  }
...
object SGDBuilder extends OptimMethodBuilder {
  ...
  override def decode[T: ClassTag](json: JObject)
    (implicit ev: TensorNumeric[T]): SGD[T] = {
    implicit val formats: DefaultFormats.type = DefaultFormats
    val learningRate = (json \ "learningRate").extract[Double]
    val learningRateDecay = (json \ "learningRateDecay").extract[Double]
    val weightDecay = (json \ "weightDecay").extract[Double]
  ...
  override def encode[T: ClassTag](opt: OptimMethod[T])
    (implicit ev: TensorNumeric[T]): JObject = {
...
class TriggerParam(
    ...
    name: String,
    doc: String,
    isValid: Trigger => Boolean)
  extends Param[Trigger](parent, name, doc, isValid) {

  def this(parent: Params, name: String, doc: String) =
    this(parent, name, doc, (_: Trigger) => true)
...
object MLParams {
  @DeveloperApi
  def getDefaultParamsReader: DefaultParamsReader.type = {
    org.apache.spark.ml.util.DefaultParamsReader
  }

  @DeveloperApi
  def getDefaultParamsWriter: DefaultParamsWriter.type = {
    org.apache.spark.ml.util.DefaultParamsWriter
  }
}