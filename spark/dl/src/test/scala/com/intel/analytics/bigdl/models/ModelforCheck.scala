...
object GoogleNet_v1_test {
  def apply(classNum: Int): Module[Double] = {
    ...
    feature1.add(SpatialConvolution(3, 64, 7, 7, 2, 2, 3, 3, 1, true).reset(Xavier)
    ...
    feature1.add(SpatialConvolution(64, 64, 1, 1, 1, 1).reset(Xavier)
      .setName("conv2/3x3_reduce"))
    feature1.add(ReLU(true).setName("conv2/relu_3x3_reduce"))
    feature1.add(SpatialConvolution(64, 192, 3, 3, 1, 1, 1, 1).reset(Xavier)
      .setName("conv2/3x3"))
    ...
    output3.add(Linear(1024, classNum).reset(Xavier).setName("loss3/classifier"))
  ...
  def inception(inputSize: Int, config: Table, namePrefix : String = "") : Module[Double] = {
    ...
    pool.add(SpatialConvolution(inputSize,
      config[Table](4)(1), 1, 1, 1, 1).reset(Xavier).setName(namePrefix + "pool_proj"))
