...
class QuantizationSpec extends FlatSpec with Matchers with BeforeAndAfter{
  ...
  "Quantize LeNet5" should "generate the same top1 accuracy" in {
    val lenetFP32Model = System.getProperty("lenet.fp32.model")
    val mnist = System.getProperty("mnist")
    ...
    int8Model.save("/tmp/model.quantized.960", overWrite = true)
  }

  "Quantize ResNet on Cifar" should "generate the same top1 accuracy" in {
    val resnetFP32Model = System.getProperty("resnet.fp32.model")
    val cifar10 = System.getProperty("cifar10")
  ...
  "Load quantized model of ResNet on Cifar" should "generate the same top1 accuracy" in {
    val lenetFP32Model = System.getProperty("lenet.fp32.model")
    val mnist = System.getProperty("mnist")
    val lenetInt8Model = System.getProperty("lenet.int8.model")
    ...
    val int8Model = Module.load(lenetInt8Model).asInstanceOf[Module[Float]]
    val int8Result = test(int8Model, evaluationSet, batchSize)
