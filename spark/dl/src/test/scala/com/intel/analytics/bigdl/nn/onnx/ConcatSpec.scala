...
class ConcatSpec extends FlatSpec with Matchers {
  ...
  "Concat" should "work" in {
    val nInputDims = 3
    val concat = Concat[Float](nInputDims = nInputDims)
    val input = T(
      Tensor[Float](1, 2, 5, 5).rand(),
      Tensor[Float](1, 2, 5, 5).rand(),
      Tensor[Float](1, 2, 5, 5).rand()
    )
    val out = concat.forward(input)
...
class ConcatSerialTest extends ModuleSerializationTest {
  override def test(): Unit = {
    val concat = Concat[Float](nInputDims = 3)
    val input = T(
      Tensor[Float](2, 5, 5).rand(),
      Tensor[Float](2, 5, 5).rand(),
      Tensor[Float](2, 5, 5).rand()
    )
