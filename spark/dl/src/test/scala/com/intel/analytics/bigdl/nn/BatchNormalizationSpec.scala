...
class BatchNormalizationSpec extends FlatSpec with Matchers {
  ...
  "BacthNormalization parameter sync double" should "work properly" in {
    ...
    val bn1 = BatchNormalization[Double](2)
    ...
    val input1 = Tensor[Double](1, 2).fill(1.0)

    val input2 = Tensor[Double](1, 2).fill(2.0)

    val inputs = Array(input1, input2)

    val gradOutput1 = Tensor[Double](1, 2).fill(2.0)
    val gradOutput2 = Tensor[Double](1, 2).fill(1.0)
