...
class TensorflowSaverSpec extends TensorflowSpecHelper {
  ...
  "AvgPooling" should "be correctly saved" in {
    val layer = SpatialAveragePooling(2, 2, 2, 2, format = DataFormat.NHWC)
  ...
  "MaxPooling" should "be correctly saved" in {
    val layer = SpatialMaxPooling(2, 2, 2, 2, format = DataFormat.NHWC)
    val input = Tensor[Float](T(T(
      T(
        T(1.0f, 2.0f, 5.0f),
        T(-3.0f, -4.0f, -7.0f),
  ...
  "CAddTableToTF" should "be correct" in {
    ...
    testMultiInput(layer, Seq(input1, input2)) should be(true)
  }
  ...
  "Batch Norm2D" should "be correctly saved" in {
    val layer = SpatialBatchNormalization(2, format = DataFormat.NHWC)
    layer.evaluate()
    layer.weight.rand(10.0, 20.0)
    layer.bias.rand()
    layer.runningVar.rand(0.9, 1.1)
    layer.runningMean.rand()
    val input = Tensor[Float](3, 4, 5, 2).rand()
    test(layer, input, "/output") should be(true)
  }
  ...
  "lenet" should "be correctly saved" in {
    ...
    val conv1 = SpatialConvolution(1, 6, 5, 5,
      format = DataFormat.NHWC).setName("conv1").inputs()
    val tanh1 = Tanh().setName("tanh1").inputs(conv1)
    val pool1 = SpatialMaxPooling(2, 2, 2, 2,
      format = DataFormat.NHWC).setName("pool1").inputs(tanh1)
    val tanh2 = Tanh().setName("tanh2").inputs(pool1)
    val conv2 = SpatialConvolution(6, 12, 5, 5,
      format = DataFormat.NHWC).setName("conv2").inputs(tanh2)
    val pool2 = SpatialMaxPooling(2, 2, 2, 2,
      format = DataFormat.NHWC).setName("output").inputs(conv2)
  ...
  private def testMultiInput(layer: AbstractModule[Table, Tensor[Float], Float],
    ...
    TensorflowSaver.saveGraphWitNodeDef(
      graph,
      inputTensors.zipWithIndex.map(t =>
