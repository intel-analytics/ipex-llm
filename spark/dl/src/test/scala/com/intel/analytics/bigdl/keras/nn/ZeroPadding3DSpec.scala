...
class ZeroPadding3DSpec extends KerasBaseSpec {
  ...
  "ZeroPadding3D" should "be the same as Keras" in {
    ...
    val layer = ZeroPadding3D[Float](Array(1, 1, 1), "CHANNEL_FIRST", inputShape = Shape(3, 4, 5, 6))
    seq.add(layer)
  ...
  "ZeroPadding3D with different padding sizes" should "be the same as Keras" in {
    val kerasCode =
      ...
      """.stripMargin
    val seq = KSequential[Float]()
    val layer = ZeroPadding3D[Float](Array(2, 1, 3), "CHANNEL_FIRST", inputShape = Shape(3, 4, 5, 6))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
      kerasCode)
  }
  ...
  "ZeroPadding3D channel_last" should "be the same as Keras" in {
    val kerasCode =
      ...
      """.stripMargin
    val seq = KSequential[Float]()
    val layer = ZeroPadding3D[Float](Array(1, 1, 1), "CHANNEL_LAST", inputShape = Shape(3, 4, 5, 6))
    seq.add(layer)
    checkOutputAndGrad(seq.asInstanceOf[AbstractModule[Tensor[Float], Tensor[Float], Float]],
      kerasCode)
  }
