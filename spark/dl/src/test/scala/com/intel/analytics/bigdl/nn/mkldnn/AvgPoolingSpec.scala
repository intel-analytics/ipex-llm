...
class AvgPoolingSpec extends BigDLSpecHelper {
  ...
  "Avg Pooling with NHWC" should "be correct" in {
    ...
    val pool = AvgPooling(3, 3, 2, 2, padH = pad, padW = pad)
    RNG.setSeed(100)
    val layer = SpatialAveragePooling[Float](3, 3, 2, 2, padH = pad, padW = pad,
      format = DataFormat.NHWC).ceil()

    layer.forward(input)

    val seq = Sequential()
    seq.add(ReorderMemory(HeapData(Array(batchSize, 28, 28, 480), Memory.Format.nhwc),
      HeapData(Array(batchSize, 480, 28, 28), Memory.Format.nhwc)))
    seq.add(pool)
    seq.add(ReorderMemory(HeapData(Array(batchSize, 14, 14, 480), Memory.Format.nhwc),
      HeapData(Array(batchSize, 480, 14, 14), Memory.Format.nhwc)))
    seq.compile(Phase.TrainingPhase, Array(HeapData(Array(batchSize, 28, 28, 480),
      Memory.Format.nhwc)))
