...
class QuantSpec extends FlatSpec with Matchers {
  ...
  "load quantized graph model" should "work correctly" in {
    ...
    val quantizedOut = quantizedModel.forward(in).toTensor
    ...
    def equalWithPrecision(t1: Tensor[Float], t2: Tensor[Float], precision: Int): Boolean = {
      ...
      for (i <- 0 until t1.nElement()) {
        ret = trunc(t1.storage().array()(t1Offset + i), precision) ==
                trunc(t2.storage().array()(t2Offset + i), precision)
      }

      ret
    }

    equalWithPrecision(out, quantizedOut, 3)
  }
  ...
  "replace cell" should "work correctly" in {
    ...
    cell.cell.asInstanceOf[Graph[Float]].modules(0).isInstanceOf[nn.Linear[Float]] should be (true)
    cell.cell = Module.quantize(cell.cell)
    cell.cell.asInstanceOf[Graph[Float]].modules(0).isInstanceOf[Linear[Float]] should be (true)
  }
  ...
  "linear perf" should "speed up" in {
    ...
    for (i <- 1 to 9) {
      ...
      model.add(BiRecurrent[Float](isSplitInput = flag, merge = JoinTable[Float](2, 2))
              .add(RnnCell[Float](inputSize, hiddenSize, HardTanh[Float](0, 20, true))))
    }
    println(model)
