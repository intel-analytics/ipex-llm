  ...
  "GlobalAveragePooling3D" should "be the same as Keras" in {
    val kerasCode =
      """
        ...
        |output_tensor = GlobalAveragePooling3D("channels_first")(input_tensor)
        |model = Model(input=input_tensor, output=output_tensor)
      """.stripMargin
    val seq = KSequential[Float]()
    val globalaveragepooling3d = GlobalAveragePooling3D[Float]("CHANNEL_FIRST", inputShape = Shape(3, 4, 5, 6))
