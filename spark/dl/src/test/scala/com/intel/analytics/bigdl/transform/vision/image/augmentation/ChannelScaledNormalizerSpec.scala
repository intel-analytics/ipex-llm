...
class ChannelScaledNormalizerSpec extends FlatSpec with Matchers {
  val resource = getClass.getClassLoader.getResource("pascal/")

  "ChannelScaledNormalizer" should "work properly" in {
    val data = ImageFrame.read(resource.getFile)
    val transformer = ChannelScaledNormalizer(100, 200, 300, 1.0) -> MatToFloats()
