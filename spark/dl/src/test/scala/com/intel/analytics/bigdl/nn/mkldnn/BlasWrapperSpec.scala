...
class BlasWrapperSpec extends BigDLSpecHelper {
  ...
  def modelWrapper(format: DataFormat = DataFormat("NCHW")) : DnnGraph = {
    val conv1 = BlasWrapper(nn.SpatialConvolution(1, 20, 5, 5)).inputs()
    ...
    DnnGraph(Array(conv1), Array(log))
  }
  ...
  "wrapper model" should "be correct" in {
    ...
    wrapper.setRuntime(new MklDnnRuntime)
    wrapper.initFwdPrimitives(Array(HeapData(inputShape, Memory.Format.nchw)), TrainingPhase)
    wrapper.initBwdPrimitives(Array(HeapData(outputShape, Memory.Format.nc)), TrainingPhase)
    wrapper.initGradWPrimitives(Array(HeapData(outputShape, Memory.Format.nc)), TrainingPhase)
