...
class COCODatasetSpec extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  "string2RLE" should "run well" in {
    ...
    val result = MaskAPI.string2RLE(compressed1, 100, 200)
    result.counts should be(arr1)
    result.height should be (100)
    result.width should be (200)

    val result2 = MaskAPI.string2RLE(compressed2, 100, 200)
  ...
  "RLE2String" should "run well" in {
    MaskAPI.RLE2String(COCORLE(arr2, 100, 200)) should be (compressed2)
    MaskAPI.RLE2String(COCORLE(arr1, 100, 200)) should be (compressed1)
  }
  ...
  "poly2RLE" should "run well" in {
    val rle = MaskAPI.poly2RLE(COCOPoly(Array(poly1)), 480, 640)
    MaskAPI.RLE2String(rle(0)) should be(
    ...
    val rle2 = MaskAPI.poly2RLE(COCOPoly(Array(poly2)), 480, 640)
    MaskAPI.RLE2String(rle2(0)) should be(
  ...
  "mergeRLEs" should "run well" in {
    val rle1 = MaskAPI.poly2RLE(COCOPoly(Array(poly1)), 480, 640)(0)
    val rle2 = MaskAPI.poly2RLE(COCOPoly(Array(poly2)), 480, 640)(0)
    MaskAPI.RLE2String(MaskAPI.mergeRLEs(Array(rle1, rle2), false)) should be(
      "la^31o>1O001N101O001O001O001N2O001O001O001O0O1000001O00000O10001O000000000O2O0000000000" +
        "1O0000000000001O00000000010O00000000001O00000000001O01O00000001O00000000001O0001O0000" +
        "0001O00000000001O0001O000001O00000000001O00000001O01O00000000001O0000000001O01O000000" +
