...
Run Spark Test 
   ...
   Run Shell                        ${submit} --master ${spark_master} --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 150g --executor-cores 28 --total-executor-cores 84 --class com.intel.analytics.bigdl.models.lenet.Train ${jar_path} -f ${mnist_data_source}  -b 12 -e 50 > 1.txt; cat 1.txt|grep "Top1Accuracy"|tail -1|cut -c114-118| awk '{if($0 > 0.985) exit 2; else exit 1}'; result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
   ...
   Run Shell                        ${submit} --master ${spark_master} --executor-cores 4 --total-executor-cores 12 --class com.intel.analytics.bigdl.models.autoencoder.Train ${jar_path} --b 12 -e 50  -f ${mnist_data_source} > 2.txt; cat 2.txt|grep 'Loss'|tail -1|cut -c211-215 |awk '{if($0 < 0.55) exit 2; else exit 1}'; result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
   Log To Console                   begin PTBWordLM
   Run Shell                        ${submit} --master ${spark_master} --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 40g --executor-memory 100g --executor-cores 8 --total-executor-cores 8 --class com.intel.analytics.bigdl.example.languagemodel.PTBWordLM ${jar_path} -f ./simple-examples/data -b 120 --numLayers 2 --vocab 10001 --hidden 650 --numSteps 35 --learningRate 0.005 -e 20 --learningRateDecay 0.001 --keepProb 0.5 --overWrite > 3.txt; cat 3.txt|grep 'Loss'|tail -1|cut -c102-107 |awk '{if($0 < 24.5) exit 2; else exit 1}'; result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
   Log To Console                   begin resnet Train
   Run Shell                        ${submit} --master ${spark_master} --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 5g --executor-memory 5g --executor-cores 8 --total-executor-cores 32 --class com.intel.analytics.bigdl.models.resnet.Train ${jar_path} -f ./cifar --batchSize 448 --optnet true --depth 20 --classes 10 --shortcutType A --nEpochs 156 --learningRate 0.1 >4.txt; cat 4.txt|grep "Top1Accuracy"|tail -1|cut -c520-524 |awk '{if($0 > 0.89) exit 2; else exit 1}'; result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
   Log To Console                   begin DLClassifierLeNet
   Run Shell                        ${submit} --master ${spark_master} --executor-cores 24 --total-executor-cores 24 --driver-memory 60g --executor-memory 200g --class com.intel.analytics.bigdl.example.MLPipeline.DLClassifierLeNet ${jar_path} -b 12 -f ./mnist --maxEpoch 10 > 5.txt; cat 5.txt|grep 'Loss'|tail -1|cut -c209-213|awk '{if($0 < 0.45) exit 2; else exit 1}';  result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
   Log To Console                   begin rnn Train
   Run Shell                        ${submit} --master ${spark_master} --driver-memory 5g --executor-memory 5g --executor-cores 12 --total-executor-cores 12 --class com.intel.analytics.bigdl.models.rnn.Train ${jar_path} -f ./ -s ./models --nEpochs 10 --checkpoint ./model/ -b 12 >6.txt; cat 6.txt|grep 'Loss'|tail -1|cut -c103-107|awk '{if($0 < 1.4) exit 2; else exit 1}'; result=$?; falg=1; if [ $result -eq $falg ];then  exit -1; fi
...
Hdfs Test Suite
   ...
   Run Shell                        mvn clean test -Dsuites=com.intel.analytics.bigdl.integration.HdfsSpec -P integration-test
   Run Shell                        mvn clean test -Dsuites=com.intel.analytics.bigdl.integration.S3Spec -P integration-test
