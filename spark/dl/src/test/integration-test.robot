...
Yarn Test Suite
   ...
   Log To Console                   begin PTBWordLM
   Run Shell                        ${submit} --master yarn --deploy-mode client --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 40g --executor-cores 10 --num-executors 3 --class com.intel.analytics.bigdl.example.languagemodel.PTBWordLM ${jar_path} -f ./simple-examples/data -b 120 --numLayers 2 --vocab 10001 --hidden 650 --numSteps 35 --learningRate 0.005 -e 1 --learningRateDecay 0.001 --keepProb 0.5 --overWrite
   Log To Console                   begin resnet Train
   ...
   Log To Console                   begin rnn Train
   Run Shell                        ${submit} --master yarn --deploy-mode client --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 5g --executor-cores 10 --num-executors 3 --class com.intel.analytics.bigdl.models.rnn.Train ${jar_path} -f ./ -s ./models --nEpochs 1 --checkpoint ./model/ -b 12
   Log To Console                   begin inceptionV1 train
   Run Shell                        ${submit} --master yarn --deploy-mode client --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 60g --executor-cores 10 --num-executors 3 --class com.intel.analytics.bigdl.models.inception.TrainInceptionV1 ${jar_path} -b 12 -f ${imagenet_test_data_source} --learningRate 0.1 -e 1
   Log To Console                   begin text classification
   Run Shell                        ${submit} --master yarn --deploy-mode client --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --driver-memory 5g --total-executor-cores 32 --executor-cores 10 --num-executors 3 --class com.intel.analytics.bigdl.example.textclassification.TextClassifier ${jar_path} --batchSize 16 --baseDir /tmp/text_data --partitionNum 32
   Log To Console                   begin DLClassifierLeNet
   Run Shell                        ${submit} --master yarn --deploy-mode client --conf "spark.serializer=org.apache.spark.serializer.JavaSerializer" --executor-cores 10 --num-executors 3 --driver-memory 60g --class com.intel.analytics.bigdl.example.MLPipeline.DLClassifierLeNet ${jar_path} -b 12 -f ./mnist --maxEpoch 1
