...
BigDL supports loading models trained and saved in BigDL. To load a BigDL model, you can use `Module.load` interface (Scala) or `Model.load` (in Python). Refer to [Model Load](APIGuide/Module/#model-load) for details.  
