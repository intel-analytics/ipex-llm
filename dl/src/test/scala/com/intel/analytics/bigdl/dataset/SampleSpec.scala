...
class SampleSpec extends FlatSpec with Matchers {
  "SampleSpec with Float Tensor input and Tensor label" should "initialize well" in {
    ...
    sample.getFeature() should be (tensorInput1)
    sample.getLabel() should be (tensorLabel1)
  }
  ...
  "SampleSpec with Float Tensor input and Tensor label" should "clone well" in {
    ...
    sample.getFeature() should be (otherSample.getFeature())
    sample.getLabel() should be (otherSample.getLabel())
  }
  "SampleSpec with Float Tensor input and Tensor label" should "copyToFeature" +
    "and copyToLabel well" in {
    val size = 4
    ...
    sample.copyToFeature(input2, 0, size*size)
    sample.copyToLabel(label2, 0, size*size)

    var i = 0
    while (i < input2.length) {
      input2(i) should be (sample.getFeature().storage().array()(i))
      i += 1
    }
    i = 0
    while (i < label2.length) {
      label2(i) should be (sample.getLabel().storage().array()(i))
      i += 1
    }
  }
}
