...
object RDDOptimizerSpec {
  ...
  val batchSize = 4 * subModuleNumber
...
class RDDOptimizerSpec extends FlatSpec with Matchers with BeforeAndAfter {
  ...
  var dataSet: RDDDataSet[(Tensor[Double], Tensor[Double])] = null
  ...
  before {
    ...
    dataSet = new RDDDataSet[(Tensor[Double], Tensor[Double])] {
      override def partitions(): RDD[_] = rdd
  ...
  "An Artificial Neural Network with MSE and LBFGS" should "be trained with good result" in {
    ...
    val optimizer = new RDDOptimizer[Double](mseModule, new MSECriterion,
      new LBFGS, pm, dataSet, Trigger.maxEpoch(5), new Metrics, subModuleNumber)
    optimizer.optimize()
  ...
  it should "be same with grad agg optimizer" in {
    ...
    val optimizer = new RDDOptimizer[Double](mseModule, new MSECriterion,
      new LBFGS, pm, dataSet, Trigger.maxIteration(17), new Metrics, subModuleNumber)
    optimizer.optimize()
  ...
  "An Artificial Neural Network with MSE and SGD" should "be trained with good result" in {
    val pm = new AllReduceParameterManager[Double](mseWeight, dataSet.partitions(), new Metrics,
      T("learningRate" -> 20.0))
    val optimizer = new RDDOptimizer[Double](mseModule, new MSECriterion,
      new SGD, pm, dataSet, Trigger.maxEpoch(5), new Metrics, subModuleNumber,
      T("learningRate" -> 20.0))
    optimizer.optimize()
  ...
  it should "be same with grad agg optimizer" in {
    val pm = new AllReduceParameterManager[Double](mseWeight, dataSet.partitions(),
      new Metrics, T("learningRate" -> 20.0))
    val optimizer = new RDDOptimizer[Double](mseModule, new MSECriterion,
      new SGD, pm, dataSet, Trigger.maxIteration(17), new Metrics, subModuleNumber,
      T("learningRate" -> 20.0))
    optimizer.optimize()
  ...
  "An Artificial Neural Network with Cross Entropy and LBFGS" should
    ...
    val optimizer = new RDDOptimizer[Double](crnModule, new ClassNLLCriterion,
      new LBFGS, pm, dataSet, Trigger.maxEpoch(3), new Metrics, subModuleNumber)
    optimizer.optimize()
  ...
  "An Artificial Neural Network with Cross Entropy and SGD" should
    ...
    val pm = new AllReduceParameterManager[Double](crnWeight, dataSet.partitions())
    val optimizer = new RDDOptimizer[Double](crnModule, new ClassNLLCriterion,
      new SGD, pm, dataSet, Trigger.maxEpoch(1), new Metrics, subModuleNumber,
      T("learningRate" -> 20.0))
    optimizer.optimize()
  ...
  it should "be same with grad agg optimizer" in {
    plusOne = 1.0
    val pm = new AllReduceParameterManager[Double](crnWeight, dataSet.partitions(), new Metrics,
      T("learningRate" -> 20.0))
    val optimizer = new RDDOptimizer[Double](crnModule, new ClassNLLCriterion,
      new SGD, pm, dataSet, Trigger.maxIteration(17), new Metrics,
      subModuleNumber, T("learningRate" -> 20.0))
    optimizer.optimize()
