...
import com.intel.analytics.dllib.lib.nn.SpatialBatchNormalization
...
class SpatialBatchNormalizationSpec extends FlatSpec with Matchers with BeforeAndAfter{
  ...
  "A SpatialBatchNormalization" should "generate correct output and gradInput" in {
    ...
    val input = torch.Tensor[Double](16, 3, 4, 4)
    var i = 0
    input.apply1(e => {RNG.uniform(0.0, 255)})
//    input.apply1(e => 0.5)
    val gradOutput = torch.Tensor[Double](16, 3, 4, 4)
    i = 0
    gradOutput.apply1(e => {i += 1; 0.1 * i})

    val gradOutput2 = torch.Tensor[Double](16, 3, 4, 4)
    i = 0
