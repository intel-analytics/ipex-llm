...
class DenseTensorSpec extends FlatSpec with Matchers {
  ...
  "One index update a multi-dimension tensor with tensor" should "copy the tensor to the subset" in {
    val t : Tensor[Double] = new DenseTensor[Double](3, 2).fill(1)
  ...
  "Tensor to BreezeMatrix" should "correct" in {
    ...
    // Tensor is row-major but breeze is col-major. So if tensor is transposed, breeze is not, vice-versa
