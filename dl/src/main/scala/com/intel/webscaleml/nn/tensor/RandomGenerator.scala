...
class RandomGenerator() {
  ...
  private val UMASK = 0x80000000L /* most significant w-r bits */
  private val LMASK = 0x7fffffffL /* least significant r bits */
  ...
  private def nextState() : this.type = {
    ...
    while (j > 1){
      j -= 1
      this.state(k) = this.state(MERSENNE_STATE_M - MERSENNE_STATE_N + k) ^ TWIST(this.state(k), this.state(k+1))
      k += 1
    }

    this.state(k) = this.state(MERSENNE_STATE_M - MERSENNE_STATE_N + k) ^ TWIST(this.state(k), this.state(0))
    this
  }
