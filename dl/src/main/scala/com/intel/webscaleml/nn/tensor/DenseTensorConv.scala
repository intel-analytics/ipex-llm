...
object DenseTensorConv{
  def fullXCorr2Dptr[@specialized(Float, Double) T](output : Storage[T], _outputOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
                     nInputRows : Int, nInputCols : Int, kernel : Storage[T], _kernelOffset : Int,
                     nKernelRows : Int, nKernelCols : Int, srow : Int, scol : Int)(implicit ev:TensorNumeric[T]) : Unit = {
...
//            output(outputOffset + kx) = output(outputOffset + kx) + z * kernel(kernelOffset - kx)
            output(outputOffset + kx) = ev.plus(output(outputOffset + kx), ev.times(z, kernel(kernelOffset - kx)))
            kx += 1
          }
  ...
  def fullConv2Dptr[@specialized(Float, Double) T](output : Storage[T], _outputOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
                    nInputRows : Int, nInputCols : Int, kernel : Storage[T], _kernelOffset : Int,
                    nKernelRows : Int, nKernelCols : Int, srow : Int, scol : Int)(implicit ev:TensorNumeric[T]) : Unit = {
    val nOuputCol = (nInputCols - 1) * scol + nKernelCols
    var inputOffset = _inputOffset
...
//            output(outputOffset + kx) = output(outputOffset + kx) + z * kernel(kernelOffset + kx)
  ...
  def validConv2Dptr[@specialized(Float, Double) T](output : Storage[T], _outputOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
                     nInputRows : Int, nInputCols : Int, kernel : Storage[T], _kernelOffset : Int,
                     nKernelRows : Int, nKernelCols : Int, srow : Int, scol : Int)(implicit ev:TensorNumeric[T]) : Unit = {
    ...
    while(yy < nOutputRow) {
      ...
      while(xx < nOutputCol) {
        ...
        while(ky < nKernelRows) {
          var kx = 0
          while(kx < nKernelCols) {
//            sum += input(inputOffset + kx) * kernel(kernelOffset - kx)
...
//        output(outputOffset) = output(outputOffset) + alpha * sum
  ...
  def validXCorr2Dptr[@specialized(Float, Double) T](output : Storage[T], _outputOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
...
//        output(outputOffset) = output(outputOffset) + alpha * sum
  ...
  def conv2d[@specialized(Float, Double) T](output : Storage[T], _outputOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
             nInputRows : Int, nInputCols : Int, kernel : Storage[T], _kernelOffset : Int, nKernelRows : Int,
             nKernelCols : Int, srow : Int, scol : Int, vf : Char, xc : Char)(implicit ev:TensorNumeric[T]) : Unit = {
    require(vf == 'F' || vf == 'V', "type of convolution can be 'V' or 'F'")
    require(xc == 'X' || xc == 'C', "type of convolution can be 'X' or 'C'")

    if(vf == 'F') {
      if (xc == 'X') {
        fullXCorr2Dptr(output, _outputOffset, alpha, input, _inputOffset, nInputRows, nInputCols, kernel,
          _kernelOffset, nKernelRows, nKernelCols, srow, scol)
      } else {
        fullConv2Dptr(output, _outputOffset, alpha, input, _inputOffset, nInputRows, nInputCols, kernel,
          _kernelOffset, nKernelRows, nKernelCols, srow, scol)
      }
    } else {
      if(xc == 'X') {
        validXCorr2Dptr(output, _outputOffset, alpha, input, _inputOffset, nInputRows, nInputCols, kernel,
          _kernelOffset, nKernelRows, nKernelCols, srow, scol)
      } else {
        validConv2Dptr(output, _outputOffset, alpha, input, _inputOffset, nInputRows, nInputCols, kernel,
          _kernelOffset, nKernelRows, nKernelCols, srow, scol)
      }
    }
  ...
  def validXCorr2DRevptr[@specialized(Float, Double) T](gradWeight : Storage[T], _gradWeightOffset : Int, alpha : T, input : Storage[T], _inputOffset : Int,
...
//        val z = output(_outputOffset + yy * kc + xx) * alpha
        ...
        while(ky < or) {
          var kx = 0
          while(kx < oc){
//            gradWeight(kx + gradWeightOffset) += z * input(kx + inputOffset)
            gradWeight(kx + gradWeightOffset) = ev.plus(gradWeight(kx + gradWeightOffset), ev.times(z, input(kx + inputOffset)))
            kx += 1
          }
          gradWeightOffset += oc
          inputOffset += ic
  ...
  def conv2Dmul[@specialized(Float, Double) T: ClassTag](alpha : T, t : Tensor[T], k : Tensor[T], srow : Int, scol : Int,
                vf : Char, xc : Char)(implicit ev:TensorNumeric[T]) : Tensor[T] = {
    ...
    conv2d(result.storage().asInstanceOf[Storage[T]], result.storageOffset() - 1, alpha, input.storage().asInstanceOf[Storage[T]], input.storageOffset() - 1,
      nInputRows, nInputCols, kernel.storage().asInstanceOf[Storage[T]], kernel.storageOffset() - 1, nKernelRows, nKernelCols,
        srow, scol, vf, xc)
    result
  }
