...
class SpatialConvolution[@specialized(Float, Double) T: ClassTag](
  ...
  val weight: Tensor[T] = torch.Tensor[T](nGroup, nOutputPlane / nGroup,
    nInputPlane / nGroup, kH, kW)
  this.gradWeight = torch.Tensor[T](nGroup, nOutputPlane / nGroup, nInputPlane / nGroup, kH, kW)
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T],
    ...
    if (input.nDimension() == 3) {
      ...
      val batchSize = input.size(1)
      if (gradWeightMM == null) {
        gradWeightMM = torch.Tensor[T]().resize(Array(batchSize, nGroup, nOutputPlane / nGroup,
          nInputPlane * kH * kW / nGroup))
