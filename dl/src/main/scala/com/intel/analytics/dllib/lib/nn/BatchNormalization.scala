...
class BatchNormalization[@specialized(Float, Double) T:ClassTag] (val nOutput: Int, val eps: Double = 1e-5, val momentum: Double = 0.1, val affine: Boolean = true)
  ...
  def updateOutputDouble(input: Array[Double], inputOffset : Int, inputStride : Int,
    ...
    while(slices.hasNext){
      ...
      if (train) {
        ...
        var i = 0
        while(i < n){
          //            println(s"$i ${i % stride2 + (f-1) * stride2 + inputOffset + (i / stride2) * inputStride}")
        ...
        i = 0
        while(i < n){
          sum += (input(i % stride2 + (f-1) * stride2 + inputOffset + (i / stride2) * inputStride) - mean) * (input(i % stride2 + (f-1) * stride2 + inputOffset + (i / stride2) * inputStride) - mean)
          i += 1
        }
        ...
        runningMean.setValue(f, ev.fromType[Double](momentum * mean + (1 - momentum) * ev.toType[Double](runningMean(Array(f)))))

        val unbiasedVar = sum / (n - 1)
        runningVar.setValue(f, ev.fromType[Double](momentum * unbiasedVar + (1 - momentum) * ev.toType[Double](runningVar.storage().array()(f - 1))))
      } else {
      ...
      var i = 0
      while(i < n){
        output(i % stride2 + (f-1) * stride2 + inputOffset + (i / stride2) * inputStride) = (input(i % stride2 + (f-1) * stride2 + inputOffset + (i / stride2) * inputStride) - mean) * invstd * w +b
        //                                                                            input(i % stride2 + inputOffset + (i / stride2) * inputStride)
        i += 1
      }
  ...
  def updateOutputFloat(input: Array[Float], inputOffset : Int, inputStride : Int,
    ...
    while(slices.hasNext){
      ...
      if (train) {
        ...
        runningMean.setValue(f, ev.fromType[Float](momentum.toFloat * mean + (1 - momentum.toFloat) * ev.toType[Float](runningMean(Array(f)))))

        val unbiasedVar = sum / (n - 1)
        runningVar.setValue(f, ev.fromType[Float](momentum.toFloat * unbiasedVar + (1 - momentum.toFloat) * ev.toType[Float](runningVar.storage().array()(f - 1))))
      } else {
  ...
  override def updateOutput(input : Tensor[T]) : Tensor[T] = {
    ...
    ev.getType() match {
      case "Double" =>
        ...
        updateOutputDouble(inputData, inputOffset, inputStride, outputData, outputOffset, outputStride, nInput, n, inputStride2)
      ...
      case "Float" =>
        ...
        val outputStride = output.stride(1)
        updateOutputFloat(inputData, inputOffset, inputStride, outputData, outputOffset, outputStride, nInput, n, inputStride2)
    }
  ...
  def backwardDouble(input: Array[Double], inputOffset : Int, inputStride : Int, inputStride2 : Int,
                     ...
                     nInput : Int, n : Int, scale : Double, gradWeight : Array[Double], gradWeightOffset : Int, gradBias : Array[Double], gradBiasOffset : Int
                      ) : Unit = {
    var mean = 0.0
    var invstd = 0.0
    ...
    while(slices.hasNext){
      ...
      val (mean, invstd) = if (train) {
        ...
        (ev.toType[Double](runningMean(Array(f))), 1 / Math.sqrt(ev.toType[Double](runningVar(Array(f))) + eps))
      }

      var sum = 0.0
      var i = 0
      while(i < n){
        val index = i % gradOutputStride2 + (f-1) * gradOutputStride2 + gradOutputOffset + (i / gradOutputStride2) * gradOutputStride
      ...
      i = 0
      while(i < n){
        val inputIndex = i % inputStride2 + (f-1) * inputStride2 + inputOffset + (i / inputStride2) * inputStride
        val gradOutputIndex = i % gradOutputStride2 + (f-1) * gradOutputStride2 + gradOutputOffset + (i / gradOutputStride2) * gradOutputStride
      ...
      if (null != gradInput) {
        ...
        if (train) {
          ...
          while(i < n){
            val inputIndex = i % inputStride2 + (f-1) * inputStride2 + inputOffset + (i / inputStride2) * inputStride
            val gradInputIndex = i % gradInputStride2 + (f-1) * gradInputStride2 + gradInputOffset + (i / gradInputStride2) * gradInputStride
          ...
          while(i < n){
            val gradInputIndex = i % gradInputStride2 + (f-1) * gradInputStride2 + gradInputOffset + (i / gradInputStride2) * gradInputStride
            val gradOutputIndex = i % gradOutputStride2 + (f-1) * gradOutputStride2 + gradOutputOffset + (i / gradOutputStride2) * gradOutputStride
            gradInput(gradInputIndex) = (gradOutput(gradOutputIndex) - gradMean - gradInput(gradInputIndex)) * invstd * w
            i += 1
          }
        } else {
          var i = 0
  ...
  def backwardFloat(input: Array[Float], inputOffset : Int, inputStride : Int, inputStride2 : Int,
                     ...
                     nInput : Int, n : Int, scale : Float, gradWeight : Array[Float], gradWeightOffset : Int, gradBias : Array[Float], gradBiasOffset : Int
                      ) : Unit = {
    var mean = 0.0f
    var invstd = 0.0f
    ...
    while(slices.hasNext){
      ...
      val (mean, invstd) = if (train) {
        ...
        (ev.toType[Float](runningMean(Array(f))), 1 / Math.sqrt(ev.toType[Float](runningVar(Array(f))) + eps.toFloat).toFloat)
      }
  ...
  def backward(input: Tensor[T], gradOutput: Tensor[T], scale: T = ev.fromType[Int](1), theGradInput : Tensor[T] = null, theGradWeight : Tensor[T] = null, theGradBias : Tensor[T] = null): Tensor[T] = {
    require(train == true, "should be in training mode when this.train is true")
    ...
    ev.getType() match {
      case "Double" =>
        ...
        if (null != theGradInput && null != theGradBias && null != theGradWeight){
          ...
          backwardDouble(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            gradInputData, gradInputOffset, gradInputStride, gradInputStride2, nInput, n, ev.toType[Double](scale), gradWeightData, gradWeightOffset, gradBiasData, gradBiasOffset)
          ...
          val gradInputStride2 = gradInputDouble.stride(2)
          backwardDouble(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            gradInputData, gradInputOffset, gradInputStride, gradInputStride2, nInput, n, ev.toType[Double](scale), null, 0, null, 0)
        } else{
          val gradWeightDouble = gradWeight.asInstanceOf[Tensor[Double]]
          val gradWeightData = gradWeightDouble.storage().array()
          val gradWeightOffset = gradWeightDouble.storageOffset() - 1
          ...
          val gradBiasOffset = gradBiasDouble.storageOffset() - 1
          backwardDouble(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            null, 0, 0, 0, nInput, n, ev.toType[Double](scale), gradWeightData, gradWeightOffset, gradBiasData, gradBiasOffset)
      ...
      case "Float" =>
        ...
        if (null != theGradInput && null != theGradBias && null != theGradWeight){
          ...
          backwardFloat(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            gradInputData, gradInputOffset, gradInputStride, gradInputStride2, nInput, n, ev.toType[Float](scale), gradWeightData, gradWeightOffset, gradBiasData, gradBiasOffset)
        } else if(null != theGradInput) {
          val gradInputFloat = theGradInput.asInstanceOf[Tensor[Float]]
          val gradInputData = gradInputFloat.storage().array()
          val gradInputOffset = gradInputFloat.storageOffset() - 1
          ...
          val gradInputStride2 = gradInputFloat.stride(2)
          backwardFloat(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            gradInputData, gradInputOffset, gradInputStride, gradInputStride2, nInput, n, ev.toType[Float](scale), null, 0, null, 0)
        } else{
          val gradWeightFloat = gradWeight.asInstanceOf[Tensor[Float]]
          val gradWeightData = gradWeightFloat.storage().array()
          val gradWeightOffset = gradWeightFloat.storageOffset() - 1
          ...
          val gradBiasOffset = gradBiasFloat.storageOffset() - 1
          backwardFloat(inputData, inputOffset, inputStride, inputStride2, gradOutputData, gradOutputOffset, gradOutputStride, gradOutputStride2,
            null, 0, 0, 0, nInput, n, ev.toType[Float](scale), gradWeightData, gradWeightOffset, gradBiasData, gradBiasOffset)
