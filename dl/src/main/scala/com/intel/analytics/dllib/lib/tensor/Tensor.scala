...
//  def repeatTensor(result: Tensor, tensor: Tensor, size: Int*)
   ...
   * @return current tensor
   */
  def set(storage : Storage[T], storageOffset : Int = 1, sizes : Array[Int] = null, strides : Array[Int] = null) : Tensor[T]
  ...
  /**
   * Return a new tensor with specified sizes. The input tensor must be contiguous, and the elements number
