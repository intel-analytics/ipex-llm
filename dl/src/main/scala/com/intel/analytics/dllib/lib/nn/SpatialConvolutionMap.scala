...
import com.intel.analytics.dllib.lib.tensor.{Storage, DenseTensorConv, torch, Tensor}
...
//  val fGradInput = torch.Tensor()
  ...
  override def reset() : Unit = {
    ...
    while(i <= connTable.size(1)){
      ninp(Array(ev.toType[Int](connTable(Array(i,2))))) = ev.plus(ninp(Array(ev.toType[Int](connTable(Array(i, 2))))), ev.fromType[Int](1))
      i = i + 1
    }
    ...
    while(k <= connTable.size(1)){
      stdv = ev.divide(ev.fromType[Int](1), ev.sqrt(ev.times(ev.fromType[Int](kW* kH), ninp(Array(ev.toType[Int](connTable(Array(k,2))))))))
    ...
    k = 1
    while(k <= bias.size(1)){
      stdv = ev.divide(ev.fromType[Int](1), ev.sqrt(ev.times(ev.fromType[Int](kW * kH), ninp(Array(k)))))
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    var dimc = 1
    var nbatch = 1
    require(input.nDimension() == 3 || input.nDimension() == 4, "3D or 4D(batch mode) tensor expected")
    ...
    require(input.size(dimw) >= kW && input.size(dimh) >= kH, "input image smaller than kernel size")
...
//            output(Array(m + 1, p + 1, j + 1, k + 1)) = z
      ...
      while (k <= nWeight) {
        ...
        //          println(s"$o $i")
        DenseTensorConv.validXCorr2Dptr[T](output.storage().asInstanceOf[Storage[T]], output.storageOffset() - 1 + (o - 1 + m*nOutputPlane) * outputH * outputW, ev.fromType[Int](1), input.storage().asInstanceOf[Storage[T]],
          input.storageOffset() - 1 + (i - 1 + m * nInputPlane) * inputW * inputH, inputH, inputW, weight.storage().asInstanceOf[Storage[T]], weight.storageOffset() - 1 + (k - 1) * kW * kH, kH, kW, dH, dW)
        k += 1
      }
      m += 1
    }
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    while(m < nbatch){
      ...
      while(k <= nkernel){
        ...
        val i = ev.toType[Int](connTable(connTableIndex))
//        println(s"o:${o} i:${i}")
        DenseTensorConv.fullConv2Dptr(gradInput.storage().asInstanceOf[Storage[Double]], gradInput.storageOffset() - 1 + (i - 1 + m * nInputPlane) * inputW * inputH,
          1.0, gradOutput.storage().asInstanceOf[Storage[Double]], gradOutput.storageOffset() - 1 + (o - 1 + m * nOutputPlane) * outputH * outputW,
          outputH, outputW, weight.storage().asInstanceOf[Storage[Double]], weight.storageOffset() - 1 + (k - 1) * weightW * weightH, weightH, weightW, dH, dW)
//        print(gradInput)
//        println()
        k += 1
      }
  ...
  override def accGradParameters(input: Tensor[T], gradOutput: Tensor[T], scale: Double = 1.0): Unit = {
    ...
    while (m <= nbatch) {
      ...
      while (k <= nOutputPlane) {
        ...
        while (l <= outputH) {
          ...
          while (n <= outputW) {
            gradOutputIndex(3) = n
            gradBias(gradBiasIndex) = ev.plus(gradBias(gradBiasIndex), ev.times(ev.fromType[Double](scale), gradOutput(gradOutputIndex)))
            n += 1
          }
          l += 1
        }
    ...
    while (m < nbatch){
      ...
      while (k <= nkernel){
        ...
        DenseTensorConv.validXCorr2DRevptr(gradWeight.storage().asInstanceOf[Storage[T]], gradWeight.storageOffset() - 1 + (k-1) * weightH * weightW,
          ev.fromType[Double](scale), input.storage().asInstanceOf[Storage[T]], input.storageOffset() - 1 + (i - 1 + m * nInputPlane ) * inputW * inputH,
          inputH, inputW, gradOutput.storage().asInstanceOf[Storage[T]], gradOutput.storageOffset() - 1 + (o - 1 + m * nOutputPlane ) * outputW * outputH,
...
object SpatialConvolutionMap{
  def full[@specialized(Float, Double) T: ClassTag](nin: Int, nout: Int)(implicit ev: TensorNumeric[T]): Tensor[T] ={
  ...
  def oneToOne[@specialized(Float, Double) T:ClassTag](nfeat: Int)(implicit ev: TensorNumeric[T]): Unit ={
  ...
  def random[@specialized(Float, Double) T:ClassTag](nin: Int, nout: Int, nto: Int)(implicit ev: TensorNumeric[T]): Tensor[T] ={
    val nker = nto * nout
    ...
    // start filling frtbl
    var i = 1
    while(i <= nout){ // fro each unit in target map
