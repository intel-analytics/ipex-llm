...
class SpatialAveragePooling[@specialized(Float, Double) T:ClassTag](
    ...
    while(k < nInputPlane){
      ...
      while(yy < outputHeight){
        ...
        while(xx < outputWidth){
          ...
          outputData(outputOffset + k * outputHeight * outputWidth + yy * outputWidth + xx) = sum / divideFactor
          xx += 1
        }
        yy += 1
      }
  ...
  override def updateOutput(input: Tensor[T]): Tensor[T] = {
    ...
    var outputHeight = if(ceilMode) math.ceil((inputHeight - kH + 2 * padH).toFloat / dH).toInt + 1 else math.floor((inputHeight - kH + 2 * padH).toFloat / dH).toInt + 1
    var outputWidth = if(ceilMode) math.ceil((inputWidth - kW + 2 * padW).toFloat / dW).toInt + 1 else math.floor((inputWidth - kW + 2 * padW).toFloat / dW).toInt + 1
    if(padW != 0 || padH != 0) {
    ...
    if (input.dim() == 3) {
      ...
      if(classTag[T] == classTag[Double]) {
        updateOutputFrameDouble(input.asInstanceOf[Tensor[Double]], output.asInstanceOf[Tensor[Double]],
          nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
          kW, kH, dW, dH)
      } else {
        updateOutputFrameFloat(input.asInstanceOf[Tensor[Float]], output.asInstanceOf[Tensor[Float]],
          nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
          kW, kH, dW, dH)
      }
    }
      ...
      while(i <= nbatch) {
        ...
        results(_i - 1) = Future {
          if(classTag[T] == classTag[Double]) {
            updateOutputFrameDouble(input(_i).asInstanceOf[Tensor[Double]], output(_i).asInstanceOf[Tensor[Double]],
              nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
              kW, kH, dW, dH)
          } else {
            updateOutputFrameFloat(input(_i).asInstanceOf[Tensor[Float]], output(_i).asInstanceOf[Tensor[Float]],
              nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
              kW, kH, dW, dH)
          }
        }
    ...
    while(k < nInputPlane){
      ...
      while(yy < outputHeight){
        ...
        while(xx < outputWidth){
          ...
          wEnd = math.min(wEnd, inputWidth)
          val divideFactor = if(countIncludePad) poolSize else (hEnd - hStart) * (wEnd - wStart)
          val z = gradOutputData(gradOutputOffset + k * outputHeight * outputWidth + yy * outputWidth + xx)
          var ky = hStart
          while(ky < hEnd){
            var kx = wStart
            while(kx < wEnd){
              gradInputData(gradInputOffset + k * inputHeight * inputWidth + ky * inputWidth + kx) += z / divideFactor
              kx += 1
            }
            ky += 1
          }
  ...
  override def updateGradInput(input: Tensor[T], gradOutput: Tensor[T]): Tensor[T] = {
    ...
    if (input.dim() == 3) {
      if(classTag[T] == classTag[Double]) {
        updateGradInputFrameDouble(gradInput.asInstanceOf[Tensor[Double]], gradOutput.asInstanceOf[Tensor[Double]],
          nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
          kW, kH, dW, dH)
      } else {
        updateGradInputFrameFloat(gradInput.asInstanceOf[Tensor[Float]], gradOutput.asInstanceOf[Tensor[Float]],
          nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
          kW, kH, dW, dH)
      }
    }
      ...
      while (i <= nBatch) {
        ...
        results(_i - 1) = Future {
          if(classTag[T] == classTag[Double]) {
            updateGradInputFrameDouble(gradInput(_i).asInstanceOf[Tensor[Double]], gradOutput(_i).asInstanceOf[Tensor[Double]],
              nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
              kW, kH, dW, dH)
          } else {
            updateGradInputFrameFloat(gradInput(_i).asInstanceOf[Tensor[Float]], gradOutput(_i).asInstanceOf[Tensor[Float]],
              nInputPlane, inputHeight, inputWidth, outputHeight, outputWidth,
              kW, kH, dW, dH)
          }
        }
  ...
  override def equals(obj : Any) : Boolean = {
    ...
    kW == other.kW && kH == other.kH && dW == other.dW && dH == other.dH && padW == other.padW && padH == other.padH &&
      ceilMode == other.ceilMode && countIncludePad == other.countIncludePad && divide == other.divide
  }
