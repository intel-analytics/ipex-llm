...
import com.intel.analytics.dllib.lib.nn.{ClassNLLCriterion, Criterion, Module}
...
trait NNParams[@specialized(Float, Double) T] extends PredictorParams {
  ...
  final val model : Param[Int => Module[T]] = new Param(this, "module factory", "neural network model")

  final val criterion : Param[Criterion[T]] = new Param(this, "criterion", "criterion that evaluate the result")

  final val state : Param[Table] = new Param(this, "state", "states to train the neural network")

  final val optMethod : Param[OptimMethod[T]] = new Param(this, "optimize method", "optimize method")

  final val optimizerType : Param[String] = new Param(this, "optimizer type", "distributed optimizer type",
    ParamValidators.inArray(Array("serial", "parallel")))

  final val batchSize : IntParam = new IntParam(this, "batch size", "how much data in one forward/backward", ParamValidators.gt(0))

  final val batchNum : IntParam = new IntParam(this, "batch number", "how many batches on one partition in one iteration")

  final val modelPath : Param[String] = new Param(this, "model path", "where to store the model")
...
class NNClassifier(override val uid: String)
  ...
  override protected def train(data: DataFrame): NNClassificationModel[Double] = {
    ...
    val trainData = data.select($(labelCol), $(featuresCol)).map{ case Row(label: Double, features: Vector) =>
      LabeledPoint(label, features.toDense)}
    val communicator = new CompressedCommunicator[Double](trainData, trainData.partitions.length)
    val offset = if($(criterion).isInstanceOf[ClassNLLCriterion[Double]]) 1 else 0
    val dataSets = new ShuffleBatchDataSet[LabeledPoint, Double](trainData, toTensor(offset)_, $(batchSize), $(batchSize), $(batchNum))
    val optimizer = getOptimizer(featureSize, dataSets, communicator)
  ...
  private def getOptimizer(featureSize : Int, dataset : DataSet[_, Double] with HasEpoch, communicator: Communicator[Double]) : Optimizer[Double] = {
    val epoch = $(state)[Int]("maxIter")
    $(optimizerType) match {
      case "serial" =>
        new GradAggEpochOptimizer[Double]($(model)(featureSize).cloneModule(), $(criterion), $(optMethod), communicator, dataset, $(state)).setMaxEpoch(epoch)
      case "parallel" =>
        new WeightAvgEpochOptimizer[Double]($(model)(featureSize).cloneModule(), $(criterion), $(optMethod), communicator, dataset, $(state)).setMaxEpoch(epoch)
      case _ =>
        throw new IllegalArgumentException
    }
  }

  private def toTensor(offset : Int)(inputs : Seq[LabeledPoint], input : Tensor[Double], target : Tensor[Double]) : (Tensor[Double], Tensor[Double]) = {
    ...
    while(i < size) {
      ...
      inputs(i).features match {
        case _ : DenseVector =>
          System.arraycopy(inputs(i).features.toArray, 0, input.storage().array(), i * featureSize, featureSize)
        case _ : SparseVector =>
          var j = 0
          while(j < featureSize) {
