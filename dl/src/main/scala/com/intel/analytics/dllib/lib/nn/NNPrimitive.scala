      ...
      if (padW > 0 || padH > 0) {
        ...
        while (y < outputHeight) {
          ...
          if (iy < 0 || iy >= inputHeight) {
            util.Arrays.fill(fInputData, dstOffset + y * outputWidth, dstOffset + (y + 1) * outputWidth, 0)
          } else {
            if(dW == 1) {
              val ix = 0 - padW + kw
              ...
              if (outputWidth - rpad - lpad <= 0) {
                ...
                if(lpad > 0) util.Arrays.fill(fInputData, dstOffset + y * outputWidth, dstOffset + y * outputWidth + lpad, 0)
                System.arraycopy(inputData, srcOffset + iy * inputWidth + ix + lpad, fInputData,
                  dstOffset + y * outputWidth + lpad, outputWidth - rpad - lpad)
                if(rpad > 0) util.Arrays.fill(fInputData, dstOffset + (y + 1) * outputWidth - rpad, dstOffset + (y + 1) * outputWidth, 0)
              }
            } else {
              var x = 0
              while(x < outputWidth) {
                ...
                if (ix < 0 || ix >= inputWidth) {
                  ...
                  fInputData(dstOffset + y * outputWidth + x) = inputData(srcOffset + iy * inputWidth + ix)
                }
                x += 1
              }
            }
  ...
  def col2im_double(
    ...
    while(nPlane < nInputPlane) {
      ...
      while(kh < kH) {
        var kw = 0
        while(kw < kW) {
          val srcOffset = nPlane * (kH * kW * outputHeight * outputWidth) + kh * (kW * outputHeight * outputWidth) +
          ...
          if (padW > 0 || padH > 0) {
              ...
              if(iy >= 0 && iy < inputHeight) {
                if(dW == 1) {
                  ...
                  while(x < outputWidth) {
                    val ix = x*dW - padW + kw
                    if(ix >= 0 && ix < inputWidth){
                      inputData(dstOffset + iy * inputWidth + ix) += fInputData(srcOffset + y * outputWidth + x)
                    }
                    x += 1
                  }
                }
            ...
            while (y < outputHeight) {
              ...
              if (dW == 1) {
                ...
                var x = 0
                while (x < outputWidth) {
                  inputData(dstOffset + iy * inputWidth + ix + x * dW) += fInputData(srcOffset + y * outputWidth + x)
                  x += 1
                }
              }
              y += 1
      ...
      val k = slices.next()
        ...
        while (i < oheight) {
          ...
          while (j < owidth) {
            ...
            gradInput(maxp + k * iwidth * iheight + gradInputOffset) += gradOutput(gradOutputOffset + k * owidth * oheight + i * owidth + j)
            j += 1
          }
          i += 1
        }
