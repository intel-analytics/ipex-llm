...
//one-based indices
class SparseTensorCsr [@specialized(Float, Double) T: ClassTag](
   var _rowIndices : Array[Int],
  ...
  require(_shape.length <= 2, s"only support 2D Csr format")

  require(_values.length == _columns.length && _rowIndices.last == _values.length() + 1, s"indices' size doesn't match tensor values")
 ...
 *
  ...
  override def setValue(d1: Int, value: T): SparseTensorCsr.this.type = {
    require(1 == this.nDimension, "invalid size")
    throw new UnsupportedOperationException(s"Unimplemented")
    this
  }

  override def setValue(d1: Int, d2: Int, value: T): SparseTensorCsr.this.type = {
    require(2 == this.nDimension, "invalid size")
    throw new UnsupportedOperationException(s"Unimplemented")
    this
  }

  override def setValue(d1: Int, d2: Int, d3: Int, value: T): SparseTensorCsr.this.type = {
    require(3 == this.nDimension, "invalid size")
    throw new UnsupportedOperationException(s"Unimplemented")
    this
  }

  override def setValue(d1: Int, d2: Int, d3: Int, d4: Int, value: T): SparseTensorCsr.this.type = {
    require(4 == this.nDimension, "invalid size")
    throw new UnsupportedOperationException(s"Unimplemented")
    this
  }

  override def setValue(d1: Int, d2: Int, d3: Int, d4: Int, d5: Int, value: T): SparseTensorCsr.this.type = {
 ...
 *
   * @return size array
   */
  override def size(): Array[Int] = if(transposed) Array(_shape(1), _shape(0)) else _shape
 ...
 *
   ...
   * @return size
   */
  override def size(dim: Int): Int = if(transposed) _shape(dim % 2) else _shape(dim - 1)
 ...
 *
  ...
  override def transpose(dim1: Int, dim2: Int): Tensor[T] = {
    require(2 == this.nDimension, "invalid size")
    val result = new SparseTensorCsr(_rowIndices, _columns, _values, _shape)
