...
object Cifar {
  ...
  def getOptim(params : Params, communicator: Communicator[Double], dataSets : DataSet[_, Double] with HasEpoch, config : Table): Optimizer[Double] ={
    ...
    params.distribute match{
      case "parallel" =>
        new WeightAvgEpochOptimizer[Double](model, getCriterion, optim, communicator, dataSets, config).setMaxEpoch(config.get[Int]("epoch").getOrElse(20))
      case "serial" =>
        new GradAggEpochOptimizer[Double](model, getCriterion, optim, communicator, dataSets, config).setMaxEpoch(config.get[Int]("epoch").getOrElse(20))
      case _ =>
  ...
  def train(params : Params) = {
    val conf = new SparkConf().setAppName(s"cifar-10, worker: $params.workerNum, partition: $params.partitionNum")
    ...
    val trainData = loadData(params.folder + "/train.vgg.txt", sc, 1, classNumber + 0.5).mapPartitions[(Tensor[Double], Tensor[Double])](toTensorRDD(stackSize, featureShape)(_)).filter(_._2.nElement()!=0).coalesce(params.partitionNum, true).persist()
    println("Estimate mean and std")
    val totalSize = trainData.count()
    println(s"train data size is ${totalSize}")


    val validationData = loadData(params.folder + "/test.vgg.txt", sc, 1, classNumber + 0.5).mapPartitions[(Tensor[Double], Tensor[Double])](toTensorRDD(stackSize, featureShape)(_)).filter(_._2.nElement()!=0).coalesce(params.partitionNum, true)
    val adjustedValidationData = trainData.zipPartitions(validationData)((train, test) => test).persist()
    ...
    val communicator = new CompressedCommunicator[Double](trainData, params.workerNum, Some(params.workerNum))
    val dataSets = new ShuffleBatchDataSet[(Tensor[Double], Tensor[Double]), Double](trainData, toTensor2(), batchSize, batchSize, batchNum)
    val optimizer = getOptim(params, communicator, dataSets, driverConfig)
  ...
  def toTensor2()(images : Seq[(Tensor[Double], Tensor[Double])], input : Tensor[Double], target : Tensor[Double]) : (Tensor[Double], Tensor[Double]) = {
    images(0)
  }
  ...
  def toTensorRDD(stackSize: Int, featureShape: Array[Int])(samples: Iterator[(Double, Array[Double])]): Iterator[(Tensor[Double], Tensor[Double])] ={
    val records = new ArrayBuffer[(Double, Array[Double])]()
    val tensors = new ArrayBuffer[(Tensor[Double], Tensor[Double])]
    def toTensor(batchSize: Int, featureShape:Array[Int])(input: Seq[(Double, Array[Double])]) : (Tensor[Double], Tensor[Double]) = {
      val size = batchSize
      val featureShape = Array(3, 32, 32)
      val result = torch.Tensor[Double](Array(size) ++ featureShape)
      val target = torch.Tensor[Double](size)
      val features = result.storage().asInstanceOf[Storage[Double]].array()
      val targets = target.storage().asInstanceOf[Storage[Double]].array()
      var i = 0
      while(i < size) {
    ...
    tensors.toIterator
//>>>>>>> some change on googlenet and cifar
  }
  ...
  def computeVar(data : RDD[(Double, Array[Double])], meanR : Double, meanG : Double, meanB : Double)
  ...
  def normalization(dataSets: RDD[(Double, Array[Double])], totalSize: Long) : RDD[(Double, Array[Double])] = {
  ...
  def nextBatch(dataSets : RDD[(Double, Array[Double])], total : Int, metaRDD : RDD[(Int, Tensor[Double], Tensor[Double])], featureShape : Array[Int],
                labelShape : Array[Int], batchSize : Int, innerLoop : Int) : RDD[(Tensor[Double], Tensor[Double])] = {
    val partitionNumber = dataSets.partitions.length
    dataSets.sample(false, innerLoop * batchSize * partitionNumber.toDouble / total, System.nanoTime()).zipPartitions(metaRDD)((dataIterator, metaIterator) => {
  ...
  def fullBatch(data : RDD[(Double, Array[Double])], total : Int, metaRDD : RDD[(Int, Tensor[Double], Tensor[Double])], featureShape : Array[Int],
                labelShape : Array[Int], batchSize : Int, innerLoop : Int) : RDD[(Tensor[Double], Tensor[Double])] = {
...
//      toTensor(dataIterator, featureShape, labelShape, batchSize, mean, std, input, target)
    })
  }

  def loadData(url : String, sc : SparkContext, numPartition : Int, classes : Double)
  : RDD[(Double, Array[Double])] = {
//    sc.binaryRecords(url, 3073).map(record => {
//      val label = record(0).toDouble + 1.0
//      val image = record.slice(1, record.length).map(_.&(0xff).toDouble)
////      rgb2yuv(image, 1024)
//      (label, image)
//    }).coalesce(numPartition, true)
  ...
  def getModel[T : ClassTag](classNumber : Int, netType : String)(implicit ev: TensorNumeric[T]): Module[T] = {
