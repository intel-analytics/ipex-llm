...
import org.apache.spark.mllib.linalg.{Matrix, Vector, DenseVector, DenseMatrix}
import breeze.linalg.{DenseMatrix => BrzDenseMatrix, DenseVector => BrzDenseVector}
import com.intel.analytics.dllib.lib.tensor.TensorType.{FloatType, DoubleType, DataType}
import com.intel.analytics.dllib.lib.tensor.{DenseTensorApply => TensorApply, DenseTensorConv => TensorConv,
    DenseTensorMath => TensorMath, DenseTensorDimApply => TensorDimApply}
...
private[tensor] class DenseTensor [@specialized(Float, Double) T: ClassTag] (
  ...
  override def size(dim : Int) : Int = {
    require(dim > 0 && dim <= this.nDimension, s"dimension ${dim} out of range of ${this.nDimension}D tensor")
    _size(dim - 1)
  }
  ...
  override def resize(size1: Int, size2: Int, size3: Int): Tensor[T] = {
    if(this.nDimension != 3 || this.size(1) != size1 || this.size(2) != size2 || this.size(3) != size3) {
  ...
  override def resize(size1: Int, size2: Int, size3: Int, size4: Int): Tensor[T] = {
    if(this.nDimension != 4 || this.size(1) != size1 || this.size(2) != size2 || this.size(3) != size3 ||
      this.size(4) != size4) {
  ...
  private[tensor] def this(d1 : Int)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](d1)), 0, Array(d1),
    Array(1), 1)
  private[tensor] def this(d1 : Int, d2 : Int)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](d1 * d2)), 0, Array(d1, d2),
    Array(d2, 1), 2)
  private[tensor] def this(d1 : Int, d2 : Int, d3 : Int)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](d1 * d2 * d3)), 0, Array(d1, d2, d3),
    Array(d3 * d2, d3, 1), 3)
  private[tensor] def this(d1 : Int, d2 : Int, d3 : Int, d4 : Int)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](d1 * d2 * d3 * d4)), 0, Array(d1, d2, d3, d4),
    Array(d4 * d3 * d2, d4 * d3, d4, 1), 4)
  private[tensor] def this(d1 : Int, d2 : Int, d3 : Int, d4 : Int, d5 : Int)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](d1 * d2 * d3 * d4 * d5)), 0, Array(d1, d2, d3, d4, d5),
    Array(d5 * d4 * d3 * d2, d5 * d4 * d3, d5 * d4, d5, 1), 5)

  private[tensor] def this(dims : Int *)(implicit ev: TensorNumeric[T]) = this(new ArrayStorage[T](new Array[T](dims.product)), 0, dims.toArray,
  ...
  private[tensor] def this(storage : Storage[T], storageOffset : Int, size : Array[Int] = null, stride : Array[Int] = null)(implicit ev: TensorNumeric[T]) = {
    this(null, 0, null, null, 0)
    if(storage != null) {
...
//      util.Arrays.fill(this.storage().array(), this.storageOffset() - 1, this.storageOffset() - 1 + this.nElement(), v)
      /*var i = this.storageOffset()-1
      while(i < this.storageOffset()-1+this.nElement()) {
        this.storage().array()(i) = v
        i += 1
      }*/
    } else {
//      TensorApply.apply1[T](this, _(_) = v)
        val func = new TensorFunc2[T] {
            override def apply(data: Array[T], index: Int): Unit =
              {data(index) = v}}
        TensorApply.apply1[T](this, func)
    }
    this
  }
...
//      TensorApply.apply1[T](this, _(_) = ev.randn())
      val func = new TensorFunc2[T] {
        override def apply(data: Array[T], index: Int): Unit =
        {data(index) = ev.randn()}}
      TensorApply.apply1[T](this, func)
//      val data = this.storage().array
//      TensorApply.apply4(this, (i) => data(i)=ev.randn())
    }
    this
  }
...
//        })
      ...
      //      val data = this.storage().array
//      TensorApply.apply4(this, (i) => data(i)=
//        if (r.draw()) {
//          ev.fromType[Int](1)
//        } else {
//          ev.fromType[Int](0)
...
//      TensorApply.apply1[T](this, _(_) = ev.rand())
      ...
      //      val data = this.storage().array
//      TensorApply.apply4(this, (i) => data(i)=ev.rand())
    }
    this
  }

  override def set(other : Tensor[T]) : Tensor[T] = {
    DenseTensor.rawSet(this, other.storage(), other.storageOffset() - 1, other.nDimension(), other.size(), other.stride())
  }
...
//    TensorApply.apply1[T](this, (data, i) => data(i) = func(data(i)))

    val func2 = new TensorFunc2[T] {
      override def apply(data: Array[T], index: Int): Unit =
      {data(index) = func(data(index)) }}
    TensorApply.apply1[T](this, func2)

    //    val data = this.storage().array
//    TensorApply.apply4(this, (i) => data(i)=func(data(i)))
    this
  }

  override def map(other : Tensor[T], func : (T, T) => T) : Tensor[T] = {
//    TensorApply.apply2[T](this, other, (data1, i1, data2, i2) => data1(i1) = func(data1(i1), data2(i2)))
    ...
    TensorApply.apply2[T](this, other, func2)
    this
  }
  ...
  override def apply(index : Int) : Tensor[T] = {
    ...
    require(_index >= 0 && _index < this._size(0), s"out of range, ${_index}: 0 to ${this._size(0)}")
  ...
  override def update(table : Table, value : T) : Unit = {
    val (tensor, offset) = subset(table)
    offset match {
      case Some(i) => tensor.storage().asInstanceOf[Storage[T]](i) = value
...
//    TensorApply.apply1[T](this, (data, i) => data(i) = if(func(data(i))) value else data(i))
    val func2 = new TensorFunc2[T] {
      override def apply(data: Array[T], index: Int): Unit =
...
//    val data = this.storage().array
//    TensorApply.apply4(this, (i) => data(i)=if(func(data(i))) value else data(i))
  }
  ...
  override def +(s : T) : Tensor[T] = TensorMath.add(s, this)
  override def +(t : Tensor[T]) : Tensor[T]= TensorMath.add(this, t)
  override def -(s : T) : Tensor[T]= TensorMath.sub(s, this)
  override def -(t : Tensor[T]) : Tensor[T]= TensorMath.sub(this, t)
  override def unary_-() : Tensor[T]= TensorMath.neg(this)
  override def /(s : T) : Tensor[T]= TensorMath.divide(s, this)
  override def /(t : Tensor[T]) : Tensor[T]= TensorMath.divide(this, t)
  override def *(s : T) : Tensor[T]= TensorMath.mul(s, this)
  override def *(t : Tensor[T]) : Tensor[T]= TensorMath.mul(this, t)
  override def sum() : T = TensorMath.sumAll(this)
  override def sum(dim : Int) = TensorMath.sum(this, dim - 1)
  override def mean() : T = TensorMath.meanAll(this)
  override def mean(dim : Int) = TensorMath.mean(this, dim - 1)
  override def max() : T = TensorMath.maxAll(this)
...
//    def func[T] = (tdata:Array[T], toffset: Int, tstride: Int, tsize: Int, vdata: Array[T], voffset: Int,vstride: Int,
//                   vsize: Int, idata:Array[T], ioffset:Int, istride:Int, isize:Int) => {
...
//      }
//      vdata(voffset) = max
//      idata(ioffset) = ev.fromType[Int](index)
//    }
//    TensorDimApply.dimApply3[T](this, values, indices, dim, func[T])
    TensorDimApply.dimApply3[T](this, values, indices, dim, (tdata, toffset, tstride, tsize, vdata, voffset,vstride,
  ...
  override def add(value : T, y : Tensor[T]) = TensorMath.cadd(this, this, value, y)
  override def add(y : Tensor[T]) = TensorMath.cadd(this, this, ev.fromType[Int](1), y)

  // Puts the result of x + value * y in current tensor
  override def add(x: Tensor[T], value: T, y: Tensor[T]): Tensor[T]= TensorMath.cadd(this, x, value, y)
...
//    TensorApply.apply3[T](this, tensor1, tensor2, (data1, offset1, data2, offset2, data3, offset3) => {
////      data1(offset1) = data1(offset1) + data2(offset2) * data3(offset3) * value
//      data1(offset1) = ev.plus(data1(offset1), ev.times(ev.times(data2(offset2), data3(offset3)), value))
//    })

    val func = new TensorFunc6[T] {
      override def apply(data1: Array[T], offset1: Int, data2: Array[T], offset2: Int, data3: Array[T], offset3: Int): Unit =
      {data1(offset1) = ev.plus(data1(offset1), ev.times(ev.times(data2(offset2), data3(offset3)), value))}}
    TensorApply.apply3[T](this, tensor1, tensor2, func)
    this
  }
...
//    TensorApply.apply3[T](this, tensor1, tensor2, (data1, offset1, data2, offset2, data3, offset3) => {
////      data1(offset1) = data1(offset1) + data2(offset2) / data3(offset3) * value
//      data1(offset1) = ev.plus(data1(offset1), ev.times(ev.divide(data2(offset2), data3(offset3)), value))
//    })
    val func = new TensorFunc6[T] {
      override def apply(data1: Array[T], offset1: Int, data2: Array[T], offset2: Int, data3: Array[T], offset3: Int): Unit =
      {data1(offset1) = ev.plus(data1(offset1), ev.times(ev.divide(data2(offset2), data3(offset3)), value))}}
  ...
  override def cmul(y : Tensor[T]) = TensorMath.cmul(this, y)
  override def mul(x : Tensor[T], value : T) = TensorMath.mul(this, x, value)
  override def mul(value : T) = TensorMath.mul(this, null, value)
  override def div(value : T) = TensorMath.div(this, null, value)

  override def conv2(kernel : Tensor[T], vf : Char = 'V') : Tensor[T]= TensorConv.conv2Dmul[T](ev.fromType[Int](1), this, kernel, 1, 1, vf, 'C')
  override def xcorr2(kernel : Tensor[T], vf : Char = 'V') : Tensor[T]= TensorConv.conv2Dmul[T](ev.fromType[Int](1), this, kernel, 1, 1, vf, 'X')

  override def addmm(v1 : T, M : Tensor[T], v2 : T, mat1 : Tensor[T], mat2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmm(this, v1, M, v2, mat1, mat2)

  override def addmm(M : Tensor[T], mat1 : Tensor[T], mat2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmm[T](this, ev.fromType[Int](1), M, ev.fromType[Int](1), mat1, mat2)

  override def addmm(mat1 : Tensor[T], mat2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmm[T](this, ev.fromType[Int](1), this, ev.fromType[Int](1), mat1, mat2)

  override def addmm(v2 : T, mat1 : Tensor[T], mat2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmm[T](this, ev.fromType[Int](1), this, v2, mat1, mat2)

  override def addmm(v1 : T, v2 : T, mat1 : Tensor[T], mat2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmm(this, v1, this, v2, mat1, mat2)

  override def addr(t1 : Tensor[T], t2 : Tensor[T]) : Tensor[T]= TensorMath.addr[T](this, ev.fromType[Int](1), this, ev.fromType[Int](1), t1, t2)
  override def addr(v2 : T, t1 : Tensor[T], t2 : Tensor[T]) : Tensor[T]= TensorMath.addr[T](this, ev.fromType[Int](1), this, v2, t1, t2)
  override def addr(v1 : T, t1 : Tensor[T], v2 : T, t2 : Tensor[T]) : Tensor[T]= TensorMath.addr(this, v1, this, v2, t1, t2)

  override def addmv(beta: T, vec1: Tensor[T], alpha: T, mat: Tensor[T], vec2: Tensor[T]): Tensor[T]=
    TensorMath.addmv(this, beta, vec1, alpha, mat, vec2)

  override def addmv(beta: T, alpha: T, mat: Tensor[T], vec2: Tensor[T]): Tensor[T]=
    TensorMath.addmv(this, beta, this, alpha, mat, vec2)
  ...
  override def uniform(args: T*): T = {
    ...
    if(args.length == 0){
//      return TensorRandom.randDouble()
      return ev.rand()
    }else if(args.length == 1){
//      return TensorRandom.randDouble()*(args(0) - 1) + 1
      return ev.plus(ev.times(ev.rand(), ev.minus(args(0), ev.fromType[Int](1))), ev.fromType[Int](1))
    }else{
      require(ev.toType[Double](ev.minus(args(0), args(1)))<=0.0, s"invalid arguments, excepted ${args(0)} <= ${args(1)}.")
//      return TensorRandom.randDouble()*(args(1) - args(0)) + args(0)
      return ev.plus(ev.times(ev.rand(), ev.minus(args(1), args(0))), args(0))
    }
  }
  ...
  override def repeatTensor(sizes: Array[Int]): Tensor[T]={
    ...
    val size = new DenseTensor(torch.storage[T](xSize.map(x => ev.fromType[Int](x)))).
      cmul(new DenseTensor(torch.storage[T](sizes.map(x => ev.fromType[Int](x))))).storage().array().map(x => ev.toType[Int](x))
  ...
  override def expand(sizes : Array[Int]) : Tensor[T]= {
    require(sizes.length == this.dim(), s"the number of dimensions provided must equal ${this.dim()}")
    ...
    while(i < tensorDim){
      if (tensorSize(i) == 1){
        ...
        throw new UnsupportedOperationException("incorrect size: only supporting singleton expansion (size=1)")
      }
      i += 1
    }
  ...
  override def addmv(alpha : T, mat : Tensor[T], vec2 : Tensor[T]) : Tensor[T]=
    TensorMath.addmv(this, ev.fromType[Int](1), this, alpha, mat, vec2)
  ...
  override def toBreezeVector(): BrzDenseVector[T] = {
    ...
    new BrzDenseVector(this.storage().array(), this.storageOffset() - 1, this.stride(1), this.nElement())
  }
  ...
  override def toMLlibMatrix(): Matrix = {
    ...
    new DenseMatrix(this.size(1), this.size(2), this.storage().array().asInstanceOf[Array[Double]], this.stride(2) == 1) // column major
  }

  override def toBreezeMatrix(): BrzDenseMatrix[T] = {
    require(this.nDimension == 2, "tensor is not 2D")
    val majorStride = if(this.stride(2) == 1) this.stride(1) else this.stride(2)
    new BrzDenseMatrix[T](this.size(1), this.size(2), this.storage().array(), this.storageOffset() - 1,
...
//    DenseTensorApply.apply3[T](this, tensor1, tensor2, (data1, offset1, data2, offset2, data3, offset3) => {
//      data1(offset1) = ev.plus(data1(offset1), ev.times(data2(offset2) ,data3(offset3)))
////      data1(offset1) = data1(offset1) + data2(offset2) * data3(offset3)
//    })
    val func = new TensorFunc6[T] {
      override def apply(data1: Array[T], offset1: Int, data2: Array[T], offset2: Int, data3: Array[T], offset3: Int): Unit =
      {data1(offset1) = ev.plus(data1(offset1), ev.times(data2(offset2) ,data3(offset3)))}}
  ...
  override def hashCode() : Int = {
    ...
    this.apply1(e => {hash = hash * seed + e.hashCode(); e})
  ...
  override def toString() : String = {
    this.nDimension match {
      ...
      case 1 =>
        ...
        this.apply1( e => {sb.append(e).append('\n'); e})
  ...
  override def diff(other: Tensor[T], count: Int, reverse : Boolean) : Boolean = {
    ...
    var result = false
    var catchNum = 0
//    DenseTensorApply.apply2[T](this, other, (data1, offset1, data2, offset2) =>{
...
//    })
    ...
    if(result == true) {
      ...
      while(i < buffer.length) {
        println(s"Find difference => this is ${buffer(i)._1} other is ${buffer(i)._2} offset is (${buffer(i)._3}/${this.nElement()}})")
        i += 1
      }
    }
...
object DenseTensor{
  ...
  private[tensor] def newWithStorage[@specialized(Float, Double) T: ClassTag](tensor : DenseTensor[T], storage : Storage[T],
                        storageOffset : Int, size : Array[Int], stride : Array[Int], ev: TensorNumeric[T]) : DenseTensor[T] = {
  ...
  private[tensor] def newWithTensor[@specialized(Float, Double) T: ClassTag](other : DenseTensor[T])(implicit ev: TensorNumeric[T]) : DenseTensor[T] = {
    val self = new DenseTensor[T]()
    DenseTensor.rawSet[T](self, other._storage, other._storageOffset, other.nDimension, other._size, other._stride)
  }

  private[tensor] def rawSet[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], storage : Storage[T], storageOffset : Int,
  ...
  private[tensor] def rawResize[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], nDim : Int, _size : Array[Int], _stride : Array[Int])
  : DenseTensor[T] = {
    ...
    while(d < nDim && !break) {
      if(_size(d) > 0) {
        ...
        if(self.nDimension > d && _stride != null && _stride(d) >= 0 && _stride(d) != self._stride(d))
          hasCorrectSize = false
      } else
        break = true
      d += 1
  ...
  private[tensor] def newClone[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T])(implicit ev: TensorNumeric[T]) : DenseTensor[T] = {
  ...
  private[tensor] def newContiguous[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T])(implicit ev: TensorNumeric[T]) : DenseTensor[T] = {
  ...
  private[tensor] def newSelect[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], _dimension : Int,
  ...
  private[tensor] def newNarrow[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], _dimension : Int,
  ...
  private[tensor] def newTranspose[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], _dimension1 : Int,
  ...
  private[tensor] def resizeAs[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], src : Tensor[_]) : Unit = {
    if(!isSameSizeAs(self, src))
      rawResize(self, src.nDimension(), src.size(), null)
  }

  private[tensor] def resize[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], sizes : Array[Int], strides : Array[Int] = null) = {
  ...
  private[tensor] def isSameSizeAs[@specialized(Float, Double) T](self : DenseTensor[T], src : Tensor[_]) : Boolean = {
  ...
  private[tensor] def set[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], other : Tensor[T]) : Tensor[T] = {
    if(self != other)
      DenseTensor.rawSet(self, other.storage, other.storageOffset, other.nDimension, other.size, other.stride)
    else
      self
  }

  private[tensor] def offsetFromIndice[@specialized(Float, Double) T](self : DenseTensor[T], indexes : Array[Int]) : Int = {
  ...
  private[tensor] def select[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], source : Tensor[T], _dimension : Int, _sliceIndex : Int): Unit = {
    var src = source
    if(src == null)
      src = self
    require(src.nDimension() > 1, "cannot select on a vector")
    require(_dimension >= 0 && _dimension < src.nDimension(), "out of range")
    require(_sliceIndex >= 0 && _sliceIndex < src.size(_dimension + 1), s"${_sliceIndex} out of range 0 to ${src.size(_dimension + 1)}")
  ...
  private[tensor] def narrow[@specialized(Float, Double) T: ClassTag](self: DenseTensor[T], source : Tensor[T], _dimension : Int, _firstIndex : Int, size : Int)
  : Unit = {
    var src = source
    if(src == null)
      src = self
  ...
  private[tensor] def transpose[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], source : Tensor[T], _dimension1 : Int, _dimension2 : Int) : Unit = {
    var src = source
    if(src == null)
      src = self
  ...
  private[tensor] def get1dTensor[@specialized(Float, Double) T: ClassTag](self : DenseTensor[T], x0 : Int)(implicit ev: TensorNumeric[T]) : DenseTensor[T] = {
    new DenseTensor(new ArrayStorage(Array(get1d(self, x0))))
  }

  private[tensor] def copy[@specialized(Float, Double) T] (self : DenseTensor[T], src : Tensor[T]) : Unit = {
//    TensorApply.apply2[T](self, src, (data1, i1, data2, i2) => data1(i1) = data2(i2))
    val func2 = new TensorFunc4[T] {
      override def apply(data1: Array[T], offset1: Int, data2: Array[T], offset2: Int): Unit =
      { data1(offset1) = data2(offset2)  }}
    TensorApply.apply2[T](self, src, func2)
  }

  private[tensor] def randperm[@specialized(Float, Double) T: ClassTag](size : Int)(implicit ev: TensorNumeric[T]) : Tensor[T] = {
    ...
    i = size - 1
    while(i > 0){
      val rand = TensorRandom.randInt(i + 1)
      val tmp = array(i)
      array(i) = array(rand)
      array(rand) = tmp
      i = i - 1
