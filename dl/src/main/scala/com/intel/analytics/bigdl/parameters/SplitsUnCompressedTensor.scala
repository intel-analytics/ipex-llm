...
class SplitsUnCompressedTensor[T: ClassTag](buffers: Array[Array[Byte]], size: Int)
  extends CompressedTensor[T] {
  import SplitsUnCompressedTensor._
    ...
    while (i < buffers.length) {
      ...
      overlap(start, curLength, offset, length) match {
        case Some((splitOffset, overlapLength)) =>
          if (buffers(i) == null) {
            buffers(i) = new Array[Byte](curLength * tLength())
          }
          UnCompressedTensor.tensorToBytes(src,
  ...
  override def compress(tensor: Tensor[T]): this.type =
    ...
    while (i < buffers.length) {
      ...
      overlap(start, curLength, srcOffset, length) match {
        case Some((splitOffset, overlapLength)) =>
          ...
          UnCompressedTensor.bytesToTensor(buffers(i), splitOffset * tLength(),
              overlapLength * tLength(), tensor, toffset + start)
        case _ =>
      }
      i += 1
    }
  ...
  override def add(data: ByteBuffer): this.type = ???

  override def parAdd(data: ByteBuffer, offset: Int,
    length: Int): this.type = ???

  override def parAdd(data: ByteBuffer): this.type = ???
  // scalastyle:on
}

object SplitsUnCompressedTensor {
  /**
