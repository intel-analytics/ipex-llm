...
import com.intel.analytics.bigdl.models.fasterrcnn.dataset.ByteImageWithRoi
...
object LocalSeqToByteImageWithRoi {
  def apply(): LocalSeqToByteImageWithRoi = new LocalSeqToByteImageWithRoi()
}

class LocalSeqToByteImageWithRoi extends Transformer[LocalSeqFilePath, ByteImageWithRoi] {
  ...
  @transient
  private var oneRecordBuffer: ByteImageWithRoi = null

  @transient
  private var recordTransformer: RecordToByteImageWithRoi = null

  override def apply(prev: Iterator[LocalSeqFilePath]): Iterator[ByteImageWithRoi] = {
    if (recordTransformer == null) recordTransformer = new RecordToByteImageWithRoi
    new Iterator[ByteImageWithRoi] {
      override def next(): ByteImageWithRoi = {
