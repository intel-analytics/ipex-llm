...
import com.intel.analytics.bigdl.models.fasterrcnn.dataset.Roidb
...
class RoidbToSeqFile(blockSize: Int, baseFileName: Path) extends
  Transformer[Roidb, String] {
  ...
  override def apply(prev: Iterator[Roidb]): Iterator[String] = {
    new Iterator[String] {
      ...
      override def next(): String = {
        ...
        while (i < blockSize && prev.hasNext) {
          ...
          baos.close()
          classLen = if (roidb.gtClasses != null) roidb.gtClasses.nElement() * 4 else 0
          ...
          if (roidb.gtClasses != null) {
            val cls = tensorToBytes(roidb.gtClasses)
            System.arraycopy(cls, 0, data, startInd, cls.length)
            startInd += cls.length
            val bbox = tensorToBytes(roidb.gtBoxes)
...
object RoidbToSeqFile {
  def apply(blockSize: Int, baseFileName: Path): RoidbToSeqFile =
    new RoidbToSeqFile(blockSize, baseFileName)
}
