...
python /ppml/trusted-big-data-ml/work/kms-client/v2/KmsApi.py \
  --api encrypt_db_automation --ip $KMS_SERVER_IP  --dbp $INPUT_DB_PATH
csv_path=$INPUT_DB_PATH.csv.encrypted
...
SGX=1 ./pal_loader bash -c "bash ./work/test-suites/DecryptFilesWithSpark-v2.sh $csv_path $KMS_SERVER_IP $LOCAL_IP" 2>&1 > spark-inside-sgx.log
fi
...
python /ppml/trusted-big-data-ml/work/kms-client/v2/KmsApi.py -api decrypt_csv_columns -ip 192.168.0.112 -dir $csv_path.col_encrypted -pkp /ppml/trusted-big-data-ml/encrypted_primary_key -dkp /ppml/trusted-big-data-ml/encrypted_data_key
