...
echo ">>> $MASTER"
ssh root@$MASTER "rm -rf $ENCLAVE_KEY_PATH && rm -rf $KEYS_PATH && rm -rf $SECURE_PASSWORD_PATH && rm -rf $DATA_PATH && mkdir -p $BIGDL_PPML_PATH"
...
for worker in ${WORKERS[@]}
  do
    echo ">>> $worker"
    ssh root@$worker "rm -rf $ENCLAVE_KEY_PATH && rm -rf $KEYS_PATH && rm -rf $SECURE_PASSWORD_PATH && rm -rf $DATA_PATH && mkdir -p $BIGDL_PPML_PATH"
...
echo ">>> $MASTER"
ssh root@$MASTER "docker pull $TRUSTED_BIGDATA_ML_IMAGE"
for worker in ${WORKERS[@]}
  do
    echo ">>> $worker"
    ssh root@$worker "docker pull $TRUSTED_BIGDATA_ML_IMAGE"
  done
...
ssh root@$MASTER "docker run -itd \
      ...
      $TRUSTED_BIGDATA_ML_IMAGE bash -c 'cd /ppml/trusted-big-data-ml && ./init.sh && ./standalone/start-spark-standalone-master-sgx.sh  && tail -f /dev/null'"
...
for worker in ${WORKERS[@]}
  do
    ...
    ssh root@$worker "docker run -itd \
          ...
          $TRUSTED_BIGDATA_ML_IMAGE bash -c 'cd /ppml/trusted-big-data-ml && ./init.sh && ./standalone/start-spark-standalone-worker-sgx.sh'"
  done
