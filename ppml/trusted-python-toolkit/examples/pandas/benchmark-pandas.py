...
@click.command()
@click.option('--size', default='1m')
@click.option('--env', default='native')
def main(size, env):

	df = pd.read_csv("/ppml/examples/pandas/RDW_" + size + ".csv")
	...
	f = open("/ppml/tests/pandas/benchmark_" + env + "_" + size, "w")
	f.write(str(time1) + '\n')
	f.write(str(time2) + '\n')
	f.write(str(time3) + '\n')
	f.write(str(time4) + '\n')
...
if __name__ == '__main__':
	...
	df = pd.read_csv("/ppml/examples/pandas/RDW_10m.csv")
	join_df = pd.read_csv("/ppml/examples/pandas/join_data.csv")
