...
while [ $(nc -z -v $FLINK_JOB_MANAGER_IP $FLINK_JOB_MANAGER_REST_PORT 2>&1 | egrep -a "succeeded" | wc -l) -eq 0 ]; do
  sleep 1
done
...
while [ $(nc -z -v $REDIS_HOST $REDIS_PORT 2>&1 | egrep -a "succeeded" | wc -l) -eq 0 ]; do
  sleep 1
done
...
while [ $(nc -z -v $FLINK_TASK_MANAGER_IP $FLINK_TASK_MANAGER_DATA_PORT 2>&1 | egrep -a "succeeded" | wc -l) -eq 0 ]; do
  sleep 1
done
