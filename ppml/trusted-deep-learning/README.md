...
**Tip:** if you want to skip building the custom image, you can use our public image `intelanalytics/bigdl-ppml-trusted-deep-learning-gramine-ref:2.2.0` for a quick start, which is provided for a demo purpose. Do not use it in production.
...
Please be noted that the `intelanalytics/bigdl-ppml-trusted-deep-learning-gramine-base:2.2.0` image relies on the `intelanalytics/bigdl-ppml-gramine-base:2.2.0` image.  

For the instructions on how to build the `gramine-base` image, check `ppml/base/README.md` in our repository.  Another option is to use our public image `intelanalytics/bigdl-ppml-gramine-base:2.2.0` for a quick start.
...
Then, use the `enclave-key.pem` and the `intelanalytics/bigdl-ppml-trusted-deep-learning-gramine-base:2.2.0` image to build your own custom image. In the process, SGX MREnclave will be made and signed without saving the sensitive enclave key inside the final image, which is safer.
...
We have included a file named `mnist.py` in our `intelanalytics/bigdl-ppml-trusted-deep-learning-gramine-ref:2.2.0` image for test purpose.  In the following sections, we will show how to run distributed PyTorch training in nodes with SGX enabled.
...
```bash
export DOCKER_IMAGE=intelanalytics/bigdl-ppml-trusted-deep-learning-gramine-ref:2.2.0
