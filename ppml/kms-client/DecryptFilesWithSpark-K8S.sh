...
export TF_MKL_ALLOC_MAX_BYTES=10737418240 && \
  /opt/jdk8/bin/java \
    ...
    --executor-cores 8 \
    --executor-memory 32g \
    --jars local://$SPARK_DECRYPT_JAR_PATH \
    ...
    --class $CLASS_PATH \
    --verbose \
    local://$SPARK_DECRYPT_JAR_PATH \
    $INPUT_DIR_PATH $KMS_SERVER_IP $KMS_SERVER_PORT $ENCRYPT_KEYS_PATH $OUTPUT_DIR_PATH 2>&1 | tee spark-decrypt-k8s-sgx.log
