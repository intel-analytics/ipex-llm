...
Enter the client container deployed in the previous step and run the below commands.
  ...
  SGX=1 ./pal_loader bash -c "bash ./work/kms-client/DecryptFilesWithSpark.sh $INPUT_DIR_PATH $ENCRYPT_KEYS_PATH $KMS_SERVER_IP $LOCAL_IP $OUT_PATH" 2>&1 > spark-inside-sgx.log
