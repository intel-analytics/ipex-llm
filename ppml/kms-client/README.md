...
Enter the client container deployed in the previous step and run the below commands.
  ...
  export OUT_PATH=YOUR_OUTPUT_SAVE_DIR # You can decide a directory path for output files, it will be created automatically
  export KMS_SERVER_PORT=3000
  SGX=1 ./pal_loader bash -c "bash ./work/kms-client/DecryptFilesWithSpark.sh $INPUT_DIR_PATH $ENCRYPT_KEYS_PATH $KMS_SERVER_IP $KMS_SERVER_PORT $LOCAL_IP $OUT_PATH" 2>&1 > spark-inside-sgx.log
  
  # Step 2. Decrypt The colums And Ouput With KMS API
  python /ppml/trusted-big-data-ml/work/kms-client/KMS_Client.py --api decrypt_csv_columns --ip $KMS_SERVER_IP --dir $OUT_PATH --pkp $ENCRYPT_KEYS_PATH/encrypted_primary_key --dkp $ENCRYPT_KEYS_PATH/encrypted_data_key
  ```
