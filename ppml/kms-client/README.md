...
# Step 2. Encrypt the Input Directory Outside SGX With KMS
...
Enter the client container deployed in the previous step and run the below commands.
  ...
  # Step 2. Decrypt The columns And Ouput With KMS API
  export APPID=YOUR_APP_ID
  export APPKEY=YOUR_APP_KEY
  ...
  ......
  [INFO] Total Elapsed Time For Columns Decryption: 940.2687749862671 s
  ```
...
- Encrypt a file with specified keys (if you have generated keys before, you will find encrypted key files named `encrypted_primary_key` and `encrypted_data_key`, and you can use them again in the following):
...
- Decrypted a file with according keys:
  ...
  ```bash
  python ./work/kms-client/KMS_Client.py -api decrypt_file -ip <KMS_SERVER_IP> [-port <KMS_SERVER_PORT>] -pkp <PRIMARYED_KEY_PATH> -dkp <DATA_KEY_PATH> -dfp <ENCRYPTED_DATA_FILE_PATH>
  ```
