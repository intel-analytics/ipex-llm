...
if [ -d ${BIGDL_ROOT}/data/ ]
then
    echo "${BIGDL_ROOT}/data/ already exists"
else
    wget $FTP_URI/analytics-zoo-data/data/object-detection-coco.zip -P ${BIGDL_ROOT}/data
    unzip -q ${BIGDL_ROOT}/data/object-detection-coco.zip -d ${BIGDL_ROOT}/data/
    wget $FTP_URI/analytics-zoo-data/data/glove/glove.6B.zip -P ${BIGDL_ROOT}/data/glove
    unzip -q ${BIGDL_ROOT}/data/glove/glove.6B.zip -d ${BIGDL_ROOT}/data/glove/glove
    wget $FTP_URI/analytics-zoo-data/data/news20/20news-18828.tar.gz -P ${BIGDL_ROOT}/data/news20/
    tar -zxvf ${BIGDL_ROOT}/data/news20/20news-18828.tar.gz -C ${BIGDL_ROOT}/data/news20/
fi
...
if [ -f ${BIGDL_ROOT}/recommendation/ncf.bigdl ]
then
    echo "${BIGDL_ROOT}/recommedation/ncf.bigdl already exists"
else
    wget $FTP_URI/analytics-zoo-models/recommendation/ncf.bigdl -P ${BIGDL_ROOT}/recommendation/
fi
...
-DMODEL_PATH=${BIGDL_ROOT}/recommendation/ncf.bigdl \
...
java -cp ./recommendation-inference/target/recommendation-inference-0.1.0-SNAPSHOT-jar-with-dependencies.jar \
-DMODEL_PATH=${BIGDL_ROOT}/recommendation/ncf.bigdl \
com.intel.analytics.bigdl.apps.recommendation.inference.SimpleDriver
...
if [ -f ${BIGDL_ROOT}/data/streaming/text-model/2.log ]
then
    echo "${BIGDL_ROOT}/data/streaming/text-model/2.log already exists"
else
    wget $FTP_URI/analytics-zoo-data/data/streaming/text-model/2.log -P ${BIGDL_ROOT}/data/streaming/text-model/2.log
fi
...
--inputFile ${BIGDL_ROOT}/data/streaming/text-model/2.log \
--embeddingFilePath ${BIGDL_ROOT}/data/glove/glove/glove.6B.300d.txt \
...
then
    echo "${BIGDL_ROOT}/flink_model/mobilenet_v1_1.0_224_frozen.pb already exists"
else
...
${BIGDL_ROOT}/apps/model-inference-examples/model-inference-flink/target/model-inference-flink-0.1.0-SNAPSHOT-jar-with-dependencies.jar  \
