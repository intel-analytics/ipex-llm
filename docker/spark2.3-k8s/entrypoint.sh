...
BIGDL_K8S_CMD="$1"
if [ -z "$BIGDL_K8S_CMD" ]; then
  echo "No command to execute has been provided." 1>&2
  exit 1
fi
shift 1

SPARK_CLASSPATH="$SPARK_CLASSPATH:${SPARK_HOME}/jars/*:${BIGDL_HOME}/dist/lib/*.jar"
...
case "$BIGDL_K8S_CMD" in
  ...
  *)
    echo "Unknown command: $BIGDL_K8S_CMD" 1>&2
    exit 1
esac
