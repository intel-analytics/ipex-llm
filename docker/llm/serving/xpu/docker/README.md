...
sudo docker run -itd \
        ...
        --device=/dev/dri \
        --name=demo-container \
        # an example for mapping the host model directory to the container
        -v /LLM_MODELS/:/llm/models/ \
        --shm-size="16g" \
        # your proxy settings
        -e http_proxy=... \ 
        -e https_proxy=... \
        # your no_proxy settings
        -e no_proxy="127.0.0.1,localhost" \
        $DOCKER_IMAGE
```

To start the service, you need to enter the container. A recommended command is:
```bash
#/bin/bash
