...
```bash
docker pull intelanalytics/bigdl-llm-cpu:2.4.0
```
to check if the image is successfully downloaded, you can use:
```powershell
docker images | sls intelanalytics/bigdl-llm-cpu:2.4.0
```
...
```bat
@echo off
set DOCKER_IMAGE=intelanalytics/bigdl-llm-cpu:2.4.0
...
```
docker pull intelanalytics/bigdl-llm-xpu:2.4.0
```
To map the xpu into the container, you need to specify --device=/dev/dri when booting the container.
An example could be:
```bash
#/bin/bash
export DOCKER_IMAGE=intelanalytics/bigdl-llm-xpu:2.4.0
export CONTAINER_NAME=my_container
export MODEL_PATH=/llm/models[change to your model path]
...
Pull image:
```
docker pull intelanalytics/bigdl-llm-serving-cpu:2.4.0
```
...
Pull image:
```
docker pull intelanalytics/bigdl-llm-serving-xpu:2.4.0
```
...
```bash
docker pull intelanalytics/bigdl-llm-finetune-lora-cpu:2.4.0
```
...
docker build \
  ...
  -t intelanalytics/bigdl-llm-finetune-lora-cpu:2.4.0 \
  -f ./Dockerfile .
```
...
docker run -itd \
 ...
 intelanalytics/bigdl-llm-finetune-cpu:2.4.0 \
 bash
```
...
```bash
docker pull intelanalytics/bigdl-llm-finetune-qlora-xpu:2.4.0
```
...
docker build \
  --build-arg http_proxy=${HTTP_PROXY} \
  --build-arg https_proxy=${HTTPS_PROXY} \
  -t intelanalytics/bigdl-llm-finetune-qlora-xpu:2.4.0 \
  -f ./Dockerfile .
```
...
docker run -itd \
   ...
   --shm-size="16g" \
   intelanalytics/bigdl-llm-fintune-qlora-xpu:2.4.0
```
