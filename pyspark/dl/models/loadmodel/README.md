...
For Caffe Inception model and Alexnet model, the command to transform the sequence file is (validation data is different with the [BigDL inception Prepare the data](https://github.com/intel-analytics/BigDL/tree/master/spark/dl/src/main/scala/com/intel/analytics/bigdl/models/inception#prepare-the-data))
...
```
  ...
  ....
```
- Please run auto pack python dependency script first to set up virtual env.
...
```shell
  ...
  # set proxy if node needs set proxy to access internet
  # http_proxy=http://...
 ...
 ${BigDL_HOME}/pyspark/dl/models/loadmodel/modelvalidator.py -t $modelType -f $folder -m $modelName   \
 --caffeDefPath $pathToCaffePrototxt --modelPath $pathToModel -b $batchSize

```

## Expected Results

You should get similar top1 accuracy as the original model.
