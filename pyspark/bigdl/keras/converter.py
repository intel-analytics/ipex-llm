...
class WeightLoader:
    ...
    def __load_weights_by_name(bmodel, kmodel, by_name=False):
        ...
        for klayer in keras_name_to_layer.values():
            if klayer.name in bigdl_name_to_layer:
                if isinstance(klayer, keras.layers.Merge): # Merge in keras doens't contain weight, but in bigdl we have.
                    continue
    ...
    def load_weights_from_hdf5(bmodel, kmodel, filepath, by_name=False):
        ...
        WeightLoader.__load_weights_by_name(bmodel, kmodel, by_name=by_name)
...
class LayerConverter:
    ...
    def create_sequential(self, klayer, kclyer):
        return DefinitionLoader.from_kmodel(klayer)
    ...
    def create_merge(self, klayer, kclayer):
        ...
        bseq = BLayer.Sequential()
        if hasattr(klayer, "layers") and klayer.layers:
