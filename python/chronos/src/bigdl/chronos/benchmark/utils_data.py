...
def generate_data(args):
    ...
    if args.dataset == 'tsinghua_electricity':
        tsdata_train, _, tsdata_test = get_public_dataset(name='tsinghua_electricity', with_split=True, test_ratio=0.1)
    elif args.dataset == 'nyc_taxi':
        tsdata_train, _, tsdata_test = get_public_dataset(name='nyc_taxi', with_split=True, test_ratio=0.1)
    else:
    ...
    if args.framework == "tensorflow":
        ...
        add_args = {}
        if args.model == "autoformer": add_args = {'time_enc':True, 'label_len':int(args.lookback/2)}
        train_loader = tsdata_train.to_torch_data_loader(batch_size=args.training_batchsize, roll=True,
                                                         lookback=args.lookback, horizon=args.horizon, **add_args)
        test_loader = tsdata_test.to_torch_data_loader(batch_size=args.inference_batchsize, roll=True, 
                                                       lookback=args.lookback, horizon=args.horizon, **add_args)
