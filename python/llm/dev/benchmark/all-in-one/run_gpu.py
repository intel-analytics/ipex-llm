...
# this code is copied from run.py and llama2 gpu example
import torch
...
def run_model(repo_id, local_model_hub=None, warm_up=1, num_trials=3, test_case='transformer_int4'):
    # TODO: make a parameter
    in_out_pairs = ['32-32', '1024-128']
    if test_case == 'transformer_int4':
        result = run_transformer_int4(repo_id, local_model_hub, in_out_pairs, warm_up, num_trials)
    elif test_case == 'optimize_model':
...
if __name__ == '__main__':
    ...
    for model in conf.repo_id:
        run_model(model, conf['local_model_hub'], conf['warm_up'], conf['num_trials'], conf['test_case'])
    ...
    df.to_csv(f'{current_dir}/results-{test_case}-{today}.csv')