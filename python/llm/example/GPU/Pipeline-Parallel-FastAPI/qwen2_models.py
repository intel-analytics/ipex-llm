from transformers.models.qwen2.modeling_qwen2 import Qwen2PreTrainedModel, Qwen2Config, Qwen2DecoderLayer, Qwen2RMSNorm, Qwen2MLP
...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        for i in range(self.config.num_hidden_layers):
            if i < self.layer_start or i >= self.layer_end:
                ...
                layers.append(Qwen2DecoderLayer(config, layer_idx=i-self.layer_start))
    ...
    def forward(
        ...
        for idx in range(self.num_layers):
            ...
            # import pdb
            # pdb.set_trace()
            # if past_key_values is None or len(past_key_values) <= idx:
            #     past_key_value = None
            # else:
            #     past_key_value = past_key_values[idx]
            # logger.info(f"rank: {self.pp_config.pp_rank}: {idx} {attention_mask, position_ids, past_key_values.get_seq_length(layer_idx=idx)}")
