...
With above command, BigDL-LLM optimized llama.cpp's executable files are already been downloaded in your conda environment.
...
To use related executable files, you should setup several environment variables first:
```bash
export CONDA_ROOT={YOUR_CONDA_ROOT} # like ~/anaconda3/envs/llm
export CPP_LIBS_PATH=$CONDA_ROOT/lib/python3.9/site-packages/bigdl-core-cpp/libs
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$CPP_LIBS_PATH
```
