...
def find_all_linear_names(args, model):
    ...
    cls = LowBitLinear if not(args.full_finetune) else torch.nn.Linear
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    # Alpaca clean
        ...
        alpaca_clean_path = "/mnt/disk1/data/alpaca-cleaned"
        print(f"Loading alpaca-cleaned from {alpaca_clean_path}")
        # dataset = load_dataset("yahma/alpaca-cleaned")
        dataset = load_dataset(alpaca_clean_path)
        dataset = dataset.map(extract_alpaca_dataset, remove_columns=['instruction'])
    # Chip2
