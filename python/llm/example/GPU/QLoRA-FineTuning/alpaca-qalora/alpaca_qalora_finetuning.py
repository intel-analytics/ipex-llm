...
def train(
    ...
    output_dir: str = "./bigdl-qalora-alpaca",
    # training hyperparams
    bf16: bool = True,  # default to bf16
    batch_size: int = 128,
    micro_batch_size: int = 2,  # default to be 2, limited by GPU memory
    num_epochs: int = 3,
    learning_rate: float = 9e-5,  # default to be 3e-5 to avoid divergence
    cutoff_len: int = 256,
    ...
    if saved_low_bit_model is not None:
        ...
        model = AutoModelForCausalLM.from_pretrained(
            base_model,
            load_in_low_bit="sym_int4", # According to the QLoRA paper, using "nf4" could yield better model quality than "int4"
            optimize_model=False,
            torch_dtype=torch.bfloat16,
    ...
    def tokenize(prompt, add_eos_token=True, padding=False):
        ...
        for i in range(len(results["input_ids"])):
            if (
                ...
                and len(results["input_ids"][i]) < cutoff_len
                and add_eos_token
            ):
    ...
    def generate_and_tokenize_prompt(data_point, padding=False):
        prompts = []
        for i in range(len(data_point["input"])):
            full_prompt = prompter.generate_prompt(
                data_point["instruction"][i],
                data_point["input"][i],
                data_point["output"][i],
            )
            prompts.append(full_prompt)
        examples_tokenized = tokenize(prompts, padding=padding)
    ...
    if val_set_size > 0:
        ...
        train_data = (
            train_val["train"].shuffle().map(lambda x: generate_and_tokenize_prompt(x, padding=False), batched=True)
        )
        val_data = (
            train_val["test"].shuffle().map(lambda x: generate_and_tokenize_prompt(x, padding=False), batched=True)
        )
    else:
