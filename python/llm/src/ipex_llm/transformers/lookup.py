...
class PromptLookupCandidateGenerator():
    ...
    def update_candidate_strategy(self, candidate_num: int, num_matches: int):
        ...
        if num_matches == self.num_output_tokens:
            self.num_output_tokens = min(self.num_output_tokens + 1, 6)
        elif candidate_num > num_matches + 0:
            self.num_output_tokens = max(self.num_output_tokens - 1, 1)
        # print(f"num_matches: {num_matches}, candidate_num: {candidate_num}, old_num_output_tokens: {old_num_output_tokens}, num_output_tokens: {self.num_output_tokens}")
...
def lookup_generate(self,
    ...
    while True:
        ...
        if step == 0:
            ...
            n_matches = ((output_ids[:, :-1] != verify_input_ids[:, 1:]).cumsum(-1) == 0).sum(-1).item()
