...
from intel_npu_acceleration_library.nn.functional import layer_norm
...
class LowBitStableLmMultiDecoderlayer(LLMBaseNNFactory):
    def __init__(
        self,
        # batch_size: int,
        # seq_len: int,
        ...
        *shapes,
        ...
        input_layernorm_biases=None,
        post_attn_layernorm_weights=None,
        post_attn_layernorm_biases=None,
        mode: str = "prefill",
        ...
        if input_layernorm_weights is None:
            ...
            input_layernorm_biases = []
            post_attn_layernorm_weights = []
            post_attn_layernorm_biases = []
            for i in range(num_layers):
                ...
                post_attn_layernorm_biases.append(
                    self.create_input_op(
                        (
                            self.hidden_size,
                        )
            ...
            input_layernorm_biases = [self.constant(w) for w in input_layernorm_biases]
            post_attn_layernorm_weights = [self.constant(w) for w in post_attn_layernorm_weights]
            post_attn_layernorm_biases = [self.constant(w) for w in post_attn_layernorm_biases]
    ...
    def attention(self,
        ...
        query_rot, key_rot = self.apply_rotary_pos_emb(
            q=query_rot,
            k=key_rot,
            cos=cos,
            sin=sin,
        ...
        # print("query_rot after rope: ", query_rot.shape)
        # print("key_rot after rope: ", key_rot.shape)
        ...
        # print("new_key_states shape: ", new_key_states.shape)
        # print("new_value_states shape: ", new_value_states.shape)
        # print("attn_output shape: ", attn_output.shape)
...
class FusedStableLmLowBitMultiDecoderlayer(torch.nn.Module):

    def __init__(
        self,
        parameters: List[Tuple[torch.Tensor]],
        input_laynorm_weights: List[torch.Tensor],
        input_laynorm_biases: List[torch.Tensor],
        post_attn_layernorm_weights: List[torch.Tensor],
        post_attn_layernorm_biases: List[torch.Tensor],
        ...
        for i in range(intra_stages):
            ...
            lm_0_weights = input_laynorm_weights[start:end]
            lm_0_biases = input_laynorm_biases[start:end]
            lm_1_weights = post_attn_layernorm_weights[start:end]
            lm_1_biases = post_attn_layernorm_biases[start:end]
            decoder = LowBitStableLmMultiDecoderlayer(
                [1, 1, num_heads * head_dim],
                input_layernorm_weights=lm_0_weights,
                input_layernorm_biases=lm_0_biases,
                post_attn_layernorm_weights=lm_1_weights,
                post_attn_layernorm_biases=lm_1_biases,
    ...
    def forward(
        ...
        hidden_states, new_keys, new_values = LowBitStableLmMultiDecoderlayer.run_decoders(
            inputs,
...
class FusedStableLmLowBitDecoderlayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.backend_cls_prefill = partial(
            LowBitStableLmMultiDecoderlayer,
            num_heads=num_heads,
            num_key_value_heads=num_key_value_heads,
            num_layers=1,
            cached_cos=cached_cos,
    ...
    def forward(
        ...
        inputs += (self.layer_norm_0_weight, self.layer_norm_0_bias, self.layer_norm_1_weight, self.layer_norm_1_bias)
...
def run_decode(
    ...
    for layer_idx in layer_indexs:
        ...
        input_layer_norm_biases.append(layer_norm_0_bias)
        post_attn_layernorm_weights.append(layer_norm_1_weight)
        post_attn_layernorm_biases.append(layer_norm_1_bias)

    multi_decoder = FusedStableLmLowBitMultiDecoderlayer(
        parameters=layer_weights,
        input_laynorm_weights=input_layer_norm_weights,
        input_laynorm_biases=input_layer_norm_biases,
        post_attn_layernorm_weights=post_attn_layernorm_weights,
        post_attn_layernorm_biases=post_attn_layernorm_biases,
...
def run_prefill(
    ...
    for layer_idx in layer_indexs:
        ...
        layer_norm_0_weight = curr_layer.input_layernorm.weight.to(torch.float16)
        print("layer_norm_0_weight size: ", layer_norm_0_weight.shape)
        layer_norm_0_bias = curr_layer.input_layernorm.bias.to(torch.float16)
        print("layer_norm_0_bias size: ", layer_norm_0_bias.size())
        layer_norm_1_weight = curr_layer.post_attention_layernorm.weight.to(torch.float16)
        layer_norm_1_bias = curr_layer.post_attention_layernorm.bias.to(torch.float16)

        new_decoderlayer = FusedStableLmLowBitDecoderlayer(
            weights,
            num_heads=num_heads,
            num_key_value_heads=num_key_value_heads,
            cached_cos=cached_cos,
...
def gen_stablelm_fused_model_forward(prefill_runner, decode_runner):
    def stablelm_fused_model_forward(
        self,
    ...
    return stablelm_fused_model_forward
