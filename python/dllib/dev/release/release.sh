...
if [ -d "${BIGDL_DIR}/python/dllib/src/bigdl_dllib_spark2.egg-info" ]; then
   rm -r ${BIGDL_DIR}/python/dllib/src/bigdl_dllib_spark2.egg-info
fi
...
if [ ${upload} == true ]; then
    upload_wheel_command="twine upload dist/bigdl_dllib_spark2-${bigdl_version}-py3-none-${verbose_pname}.whl"
    echo "Uploading wheel with this command: $upload_wheel_command"
    $upload_wheel_command
    if [ "$platform" == "linux" ]; then
        upload_source_command="twine upload dist/bigdl-dllib-spark2-${bigdl_version}.tar.gz"
