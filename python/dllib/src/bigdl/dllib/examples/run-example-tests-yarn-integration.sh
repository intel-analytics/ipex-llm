...
start=$(date "+%s")
#run the example
python ${BIGDL_ROOT}/python/orca/examples/orca/data/spark_pandas.py \
...
#run the example
rm -rf ./data
python ${BIGDL_ROOT}/python/examples/orca/learn/pytorch/cifar10/cifar10.py \
...
rm -rf ./data
rm -rf ./runs
python ${BIGDL_ROOT}/python/examples/orca/learn/pytorch/fashion_mnist/fashion_mnist.py \
  --cluster_mode 'yarn-client' \
  --epochs 1 \
  --batch_size 256
exit_status=$?
...
python ${BIGDL_ROOT}/python/examples/orca/learn/pytorch/super_resolution/super_resolution.py \
  --cluster_mode 'yarn-client'\
...
ls /tmp/imagenet
python ${BIGDL_ROOT}/python/examples/orca/torchmodel/train/imagenet/main.py \
  /tmp/imagenet \
  --max_epochs 1 \
  --batch-size 256 \
...
python ${BIGDL_ROOT}/python/examples/orca/torchmodel/train/mnist/main.py \
...
ls /tmp/dogscats
python ${BIGDL_ROOT}/python/examples/orca/torchmodel/train/resnet_finetune/resnet_finetune.py \
  /tmp/dogscats \
  --deploy-mode 'yarn-client'
exit_status=$?
if [ $exit_status -ne 0 ]; then
