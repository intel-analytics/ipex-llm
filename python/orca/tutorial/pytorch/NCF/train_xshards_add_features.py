...
from bigdl.orca.data.pandas import merge
from bigdl.orca.data.transformer import StringIndexer

def preprocess_data():   
    data_left = read_csv(
        Config["dataset"]+"/users.dat", 
    ...
    data_left=strid.fit_transform(data_left)
    strid.setInputCol('zipcode')
    data_left=strid.fit_transform(data_left)
    
    #merge two XShards
    data_X=merge(data_left,data_right,how='right',on='user')
...
def transform_to_dict(data):
    ...
    data_XY=pd.DataFrame(data=features_fill,columns=clms)
    ...
    train_data, test_data={"x": train_data[:,:-1].astype(np.int64), "y": train_data[:,-1].astype(np.float)}, {"x": test_data[:,:-1].astype(np.int64), "y": test_data[:,-1].astype(np.float)}  
