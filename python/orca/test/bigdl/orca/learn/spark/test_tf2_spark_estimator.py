...
class TestTFEstimator(TestCase):
    ...
    #     batch_size = 320
    #     try:
    #         est = Estimator.from_keras(model_creator=model_creator, workers_per_node=2, backend="spark")
    #
    #         est.fit(train_data_creator,
    #                           epochs=1,
    #                           batch_size=batch_size,
    #                           steps_per_epoch=5)
    #         print("start saving")
    #         est.save_weights("/tmp/cifar10_keras.h5")
    #         est.load_weights("/tmp/cifar10_keras.h5")
    #         print("save success")
    #     finally:
