...
def parse_args():
    ...
    # parser.add_argument(
    #     "--per_executor_eval_batch_size",
    #     type=int,
    #     default=8,
    #     help="Batch size (per executor) for the evaluation dataloader.",
    # )
    parser.add_argument(
        "--learning_rate",
        type=float,
...
def train_loader_creator(config, batch_size):
    ...
    # config = AutoConfig.from_pretrained(config.model_name_or_path, num_labels=num_labels, finetuning_task=config.task_name)
...
class MyMainCallback(MainCallback):
    ...
    def on_iter_forward(self, runner):
        ...
        # print("output is: ", runner.output)
        # Compute loss
        runner.loss = output.loss
...
class StepModelCheckpointCallback(ModelCheckpoint):
    ...
    def after_train_iter(self, runner):
        ...
        if (runner.global_step + 1) % self.step_frequency == 0:
...
def main():
    ...
    # eval_dataloader = DataLoader(eval_dataset, collate_fn=custom_collate, batch_size=args.per_executor_batch_size)
    ...
    if args.checkpointing_steps is not None:
        if checkpointing_steps == "epoch":
            ...
            callbacks.append(
                StepModelCheckpointCallback(filepath=os.path.join(args.output_dir, "step_{step}"),
                                            save_weights_only=True,
