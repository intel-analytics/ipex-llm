...
def get_checkpoint_from_callbacks(callbacks):
...
#         fs = pa.hdfs.connect(host=host_port[0], port=int(host_port[1]))
#         if not fs.exists(remote_dir):
#             raise Exception("The remote directory {} does not exist".format(remote_dir))
...
#             s3_client.download_file(bucket, key, os.path.join(local_dir, file_name))
#     else:
#         if remote_dir.startswith("file://"):
#             remote_dir = remote_dir[len("file://"):]
#         import shutil
...
# def get_remote_files_to_local_dir(remote_path_prefix, local_dir):
...
#             s3_client.download_file(bucket, file_key, os.path.join(local_dir, file_name))
#         return os.path.join(local_dir, prefix)
#     else:
#         if remote_path_prefix.startswith("file://"):
#             remote_path_prefix = remote_path_prefix[len("file://"):]
#         return remote_path_prefix