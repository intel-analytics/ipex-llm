...
class TestTable(TestCase):
    ...
    def test_apply(self):
        ...
        invalidInputError([v + 1 for v in col1_values] == updated_col1_values, "col1_values error")
        ...
        invalidInputError(out_values == ["xxxx"] * len(out_values), "out_values error")
    ...
    def test_apply_with_data(self):
        ...
        invalidInputError(feature_tbl.filter(col("out") == 2).size() == 3, "filter error")
        invalidInputError(feature_tbl.filter(col("out") == 1).size() == 1, "filter error")
        invalidInputError(feature_tbl.filter(col("out") == 0).size() == 1, "filter error")

    def test_fillna_int(self):
        file_path = os.path.join(self.resource_path, "parquet/data1.parquet")
        feature_tbl = FeatureTable.read_parquet(file_path)
        filled_tbl = feature_tbl.fillna(5, ["col_2", "col_3"])
        invalidInputError(isinstance(filled_tbl, FeatureTable),
                          "filled_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_2 is null").count() != 0 and
                          feature_tbl.df.filter("col_3 is null").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(filled_tbl.df.filter("col_2 == 5").count() == 1,
                          "col_2 null values should be filled with 5")
        invalidInputError(filled_tbl.df.filter("col_3 == 5").count() == 1,
                          "col_3 null values should be filled with 5")
    ...
    def test_fillna_double(self):
        ...
        invalidInputError(feature_tbl.df.filter("col_2 is null").count() != 0 and
                          feature_tbl.df.filter("col_3 is null").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(filled_tbl.df.filter("col_2 is null").count() == 0,
                          "col_2 null values should be filled")
        invalidInputError(filled_tbl.df.filter("col_3 is null").count() == 0,
                          "col_3 null values should be filled")
        filled_tbl = feature_tbl.fillna(5, ["col_2", "col_3"])
        invalidInputError(filled_tbl.df.filter("col_2 == 5").count() == 1,
                          "col_2 null values should be filled with 5")
        invalidInputError(filled_tbl.df.filter("col_3 == 5").count() == 1,
    ...
    def test_fillna_long(self):
        ...
        invalidInputError(feature_tbl.df.filter("col_2 is null").count() != 0 and
                          feature_tbl.df.filter("col_3 is null").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(filled_tbl.df.filter("col_1 is null").count() == 0,
                          "col_1 null values should be filled")
        invalidInputError(filled_tbl.df.filter("col_2 is null").count() == 0,
                          "col_2 null values should be filled")
        invalidInputError(filled_tbl.df.filter("col_3 is null").count() == 0,
                          "col_3 null values should be filled")
    ...
    def test_fillna_string(self):
        ...
        invalidInputError(isinstance(filled_tbl, FeatureTable),
                          "filled_tbl should be a FeatureTable")
        invalidInputError(filled_tbl.df.filter("col_4 is null").count() == 0,
                          "col_4 null values should be filled")
        invalidInputError(filled_tbl.df.filter("col_5 is null").count() == 0,
                          "col_5 null values should be filled")
    ...
    def test_filter_by_frequency(self):
        ...
        invalidInputError(tbl.to_spark_df().count() == 2,
                          "the count of frequency >=2 should be 2")
    ...
    def test_gen_string_idx(self):
        ...
        invalidInputError(string_idx_list[0].size() == 3, "col_4 should have 3 indices")
        invalidInputError(string_idx_list[1].size() == 2, "col_5 should have 2 indices")
        with tempfile.TemporaryDirectory() as local_path:
            for str_idx in string_idx_list:
                str_idx.write_parquet(local_path)
                str_idx_log = str_idx.log(["id"])
                invalidInputError(str_idx.df.filter("id == 1").count() == 1,
                                  "id in str_idx should = 1")
                invalidInputError(str_idx_log.df.filter("id == 1").count() == 0,
                                  "id in str_idx_log should != 1")
            invalidInputError(os.path.isdir(local_path + "/col_4.parquet"), "dir error")
            invalidInputError(os.path.isdir(local_path + "/col_5.parquet"), "dir error")
            new_col_4_idx = StringIndex.read_parquet(local_path + "/col_4.parquet")
            invalidInputError("col_4" in new_col_4_idx.df.columns,
                              "col_4 should be a column of new_col_4_idx")
            with self.assertRaises(Exception) as context:
    ...
    def test_gen_string_idx_dict(self):
        ...
        invalidInputError(string_idx_list[0].size() == 3, "col_4 should have 3 indices")
        invalidInputError(string_idx_list[1].size() == 1, "col_5 should have 1 indices")
    ...
    def test_gen_reindex_mapping(self):
        ...
        invalidInputError(index_tbls[0].size() == 3, "size error")
        invalidInputError(index_tbls[1].size() == 2, "size error")
    ...
    def test_gen_string_idx_union(self):
        ...
        string_idx_list1 = feature_tbl \
            .gen_string_idx(["col_4", 'col_5'],
                            freq_limit=1)
        invalidInputError(string_idx_list1[0].size() == 3, "col_4 should have 3 indices")
        invalidInputError(string_idx_list1[1].size() == 2, "col_5 should have 2 indices")
        new_tbl1 = feature_tbl.encode_string(['col_4', 'col_5'], string_idx_list1)
        invalidInputError(new_tbl1.max("col_5").to_list("max")[0] == 2,
                          "col_5 max value should be 2")

        string_idx_list2 = feature_tbl \
            .gen_string_idx(["col_4", {"src_cols": ["col_4", "col_5"], "col_name": 'col_5'}],
                            freq_limit=1)
        invalidInputError(string_idx_list2[0].size() == 3, "col_4 should have 3 indices")
        invalidInputError(string_idx_list2[1].size() == 4, "col_5 should have 4 indices")
        new_tbl2 = feature_tbl.encode_string(['col_4', 'col_5'], string_idx_list2)
        invalidInputError(new_tbl2.max("col_5").to_list("max")[0] == 4,
                          "col_5 max value should be 4")

        string_idx_3 = feature_tbl \
            .gen_string_idx({"src_cols": ["col_4", "col_5"], "col_name": 'col_5'}, freq_limit=1)
        invalidInputError(string_idx_3.size() == 4, "col_5 should have 4 indices")
        new_tbl3 = feature_tbl.encode_string('col_5', string_idx_3)
        invalidInputError(new_tbl3.max("col_5").to_list("max")[0] == 4,
                          "col_5 max value should be 4")
    ...
    def test_gen_string_idx_split(self):
        ...
        invalidInputError(string_idx_1.size() == 4, "list1 should have 4 indices")

        new_tbl1 = tbl.encode_string('list1', string_idx_1, do_split=True, sep=",")
        invalidInputError(isinstance(new_tbl1.to_list("list1"), list),
        ...
        invalidInputError(isinstance(new_tbl3.to_list("list1")[0], int),
    ...
    def test_clip(self):
        ...
        invalidInputError(isinstance(clip_tbl, FeatureTable),
                          "clip_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_1 < 2").count() != 0 and
                          feature_tbl.df.filter("col_2 < 2").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(clip_tbl.df.filter("col_1 < 2").count() == 0, "col_1 should >= 2")
        invalidInputError(clip_tbl.df.filter("col_2 < 2").count() == 0, "col_2 should >= 2")
        invalidInputError(clip_tbl.df.filter("col_3 < 2").count() == 0, "col_3 should >= 2")
        with self.assertRaises(Exception) as context:
        ...
        invalidInputError(isinstance(clip_tbl, FeatureTable), "clip_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_1 > 1").count() != 0 and
                          feature_tbl.df.filter("col_2 > 1").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(clip_tbl.df.filter("col_1 > 1").count() == 0, "col_1 should <= 1")
        invalidInputError(clip_tbl.df.filter("col_2 > 1").count() == 0, "col_2 should <= 1")
        invalidInputError(clip_tbl.df.filter("col_3 > 1").count() == 0, "col_3 should <= 1")

        feature_tbl = FeatureTable.read_parquet(file_path)
        clip_tbl = feature_tbl.clip(["col_1", "col_2", "col_3"], min=0, max=1)
        invalidInputError(isinstance(clip_tbl, FeatureTable), "clip_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_1 > 1 or col_1 < 0").count() != 0 and
                          feature_tbl.df.filter("col_2 > 1 or col_2 < 0").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(clip_tbl.df.filter("col_1 < 0").count() == 0, "col_1 should >= 0")
        invalidInputError(clip_tbl.df.filter("col_2 > 1").count() == 0, "col_2 should <= 1")
        invalidInputError(clip_tbl.df.filter("col_3 < 0 or col_3 > 1").count() == 0,
                          "col_3 should >=0 and <= 1")

    def test_dropna(self):
        file_path = os.path.join(self.resource_path, "parquet/data1.parquet")
        feature_tbl = FeatureTable.read_parquet(file_path)
        dropped_tbl = feature_tbl.dropna(["col_1", "col_4"])
        invalidInputError(isinstance(dropped_tbl, FeatureTable),
                          "dropped_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_1 is null").count() != 0 and
                          feature_tbl.df.filter("col_4 is null").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(dropped_tbl.df.filter("col_1 is null").count() == 0,
                          "col_1 null values should be dropped")
        invalidInputError(dropped_tbl.df.filter("col_4 is null").count() == 0,
                          "col_4 null values should be dropped")
        invalidInputError(0 < dropped_tbl.df.count() < feature_tbl.df.count(),
                          "the number of rows should be decreased")

        dropped_tbl = feature_tbl.dropna(["col_1", "col_4"], how="all")
        invalidInputError(dropped_tbl.df.filter("col_1 is null and col_4 is null").count() == 0,
                          "col_1 and col_4 should not both have null values")
        dropped_tbl = feature_tbl.dropna(["col_2", "col_4"], how="all")
        invalidInputError(dropped_tbl.df.filter("col_2 is null").count() > 0,
        ...
        invalidInputError(dropped_tbl.df.filter("col_3 is null and col_5 is null").count() == 0,
    ...
    def test_filter(self):
        ...
        invalidInputError(filtered_tbl.size() == 3, "Only 3 out of 5 rows has value 1 for col_1")
        filtered_tbl2 = feature_tbl.filter(
            (feature_tbl.col("col_1") == 1) & (feature_tbl.col_2 == 1))
        invalidInputError(filtered_tbl2.size() == 1,
                          "Only 1 out of 5 rows has value 1 for col_1 and col_2")
    ...
    def test_random_split(self):
        ...
        invalidInputError(cnt == cnt1 + cnt2,
                          "size of full table should equal to sum of size of splited ones")
    ...
    def test_rename(self):
        ...
        invalidInputError(isinstance(rename_tbl, FeatureTable),
                          "rename_tbl should be a FeatureTable")
        invalidInputError("col_1" in feature_tbl.df.columns,
                          "feature_tbl should not be changed")
        invalidInputError("new_col1" in cols,
                          "new_col1 should be a column of the renamed tbl.")
        invalidInputError("new_col4" in cols,
                          "new_col4 should be a column of the renamed tbl.")

    def test_log(self):
        file_path = os.path.join(self.resource_path, "parquet/data1.parquet")
        feature_tbl = FeatureTable.read_parquet(file_path)
        log_tbl = feature_tbl.log(["col_1", "col_2", "col_3"])
        invalidInputError(isinstance(log_tbl, FeatureTable),
                          "log_tbl should be a FeatureTable")
        invalidInputError(feature_tbl.df.filter("col_1 == 1").count() != 0 and
                          feature_tbl.df.filter("col_2 == 1").count() != 0,
                          "feature_tbl should not be changed")
        invalidInputError(log_tbl.df.filter("col_1 == 1").count() == 0,
                          "col_1 should != 1")
        invalidInputError(log_tbl.df.filter("col_2 == 1").count() == 0,
                          "col_2 should != 1")
        invalidInputError(log_tbl.df.filter("col_3 == 1").count() == 0,
                          "col_3 should != 1")
    ...
    def test_merge(self):
        ...
        invalidInputError("col_1" not in merged_tbl.df.columns,
                          "col_1 shouldn't be a column of merged_tbl")
        invalidInputError("int_cols" in merged_tbl.df.columns,
                          "int_cols should be a column of merged_tbl")
        invalidInputError("col_1" in feature_tbl.df.columns,
                          "col_1 should be a column of feature_tbl")
    ...
    def test_norm(self):
        ...
        invalidInputError(max_value <= 1, "col_2 shouldn't be more than 1 after normalization")
        invalidInputError(min_value >= 0, "col_2 shouldn't be less than 0 after normalization")
        ...
        min_value = scaled2.df.select("col2-col3") \
            ...
            .rdd.map(lambda row: row['min']).collect()[0]
        invalidInputError(max_value[0] <= 1 and max_value[1] <= 1,
                          "col2-col3 shouldn't be more than 1 after normalization")
        invalidInputError(min_value[0] >= 0 and min_value[1] >= 0,
    ...
    def test_cross(self):
        ...
        invalidInputError(max_value <= 100,
                          "cross value shouldn't be more than 100 after cross")
        invalidInputError(min_value > 0,
                          "cross value shouldn't be less than 0 after cross")
    ...
    def test_add_negative_items(self):
        ...
        dft = tbl.df
        invalidInputError(tbl.size() == 12, "size error")
        invalidInputError(dft.filter("label == 1").count() == 6, "size error")
        invalidInputError(dft.filter("label == 0").count() == 6, "size error")
    ...
    def test_add_hist_seq(self):
        ...
        invalidInputError(tbl.size() == 8, "size error")
        invalidInputError(tbl.df.filter(col("name") == "alice").count() == 2, "size error")
        invalidInputError(tbl.df.filter("name like '%jack'").count() == 6, "size error")
        invalidInputError("item_hist_seq" in tbl.df.columns, "col error")
        invalidInputError(tbl2.size() == 2, "size error")
        invalidInputError(tbl2.df.filter(col("name") == "alice").count() == 1, "size error")
        invalidInputError(tbl2.df.filter("name like '%jack'").count() == 1, "size error")
        invalidInputError("item_hist_seq" in tbl2.df.columns, "col error")
    ...
    def test_gen_neg_hist_seq(self):
        ...
        invalidInputError(tbl.df.select("neg_item_hist_seq").count() == 3, "size error")
    ...
    def test_add_value_features(self):
        ...
        invalidInputError(tbl3.df.select("category_hist_seq").count() == 3, "size error")
        invalidInputError(tbl3.df.select("neg_category_hist_seq").count() == 3, "size error")
        invalidInputError(tbl3.df.filter("name like '%alice%'")
                          .select("neg_category_hist_seq").count() == 1, "size error")
        invalidInputError(tbl3.df.filter("name == 'rose'")
                          .select("neg_category_hist_seq").count() == 1, "size error")
    ...
    def test_reindex(self):
        ...
        invalidInputError(reindexed.filter(col("col_4") == 0).size() == 2, "size error")
        invalidInputError(reindexed.filter(col("col_4") == 1).size() == 2, "size error")
        invalidInputError(reindexed.filter(col("col_5") == 0).size() == 1, "size error")
        invalidInputError(reindexed.filter(col("col_5") == 1).size() == 3, "size error")
    ...
    def test_pad(self):
        ...
        invalidInputError(dft1.filter("size(matrix) = 4").count() == 3, "size error")
        invalidInputError(dft1.filter("size(list) = 4").count() == 3, "size error")
        invalidInputError(tbl2.df.filter("size(list_mask) = 4").count() == 3, "size error")
        invalidInputError(tbl2.df.filter("size(list_mask) = 2").count() == 0, "size error")
        invalidInputError("list_mask" in tbl2.df.columns, "size error")
        invalidInputError(tbl3.df.filter("size(str_list) = 2").count() == 3, "size error")
    ...
    def test_median(self):
        ...
        invalidInputError(isinstance(median_tbl, FeatureTable),
                          "median_tbl should be a FeatureTable")
        invalidInputError(median_tbl.df.count() == 3,
                          "the number of rows of median_tbl should be equal to "
                          "the number of specified columns")
        invalidInputError(median_tbl.df.filter("column == 'col_1'").count() == 1,
                          "col_1 should exist in 'column' of median_tbl")
        invalidInputError(median_tbl.df.filter("column == 'col_2'")
    ...
    def test_cast(self):
        ...
        tbl = FeatureTable(df)
        tbl = tbl.cast("a", "int")
        invalidInputError(dict(tbl.df.dtypes)['a'] == "int",
                          "column a should be now be cast to integer type")
        tbl = tbl.cast("a", "float")
        invalidInputError(dict(tbl.df.dtypes)['a'] == "float",
                          "column a should be now be cast to float type")
        tbl = tbl.cast(["b", "c"], "double")
        invalidInputError(dict(tbl.df.dtypes)['b'] == dict(tbl.df.dtypes)['c'] == "double",
                          "column b and c should be now be cast to double type")
        tbl = tbl.cast(None, "float")
        invalidInputError(dict(tbl.df.dtypes)['name'] == dict(tbl.df.dtypes)['a']
                          == dict(tbl.df.dtypes)['b'] == dict(tbl.df.dtypes)['c'] == "float",
    ...
    def test_select(self):
        ...
        invalidInputError("col_1" in select_tbl.df.columns, "col_1 shoul be selected")
        invalidInputError("col_2" in select_tbl.df.columns, "col_2 shoud be selected")
        invalidInputError("col_3" not in select_tbl.df.columns,
                          "col_3 shoud not be selected")
        invalidInputError(feature_tbl.size() == select_tbl.size(),
    ...
    def test_create_from_dict(self):
        ...
        invalidInputError('id' in tbl.df.columns, "id should be one column in the stringindex")
        invalidInputError('letter' in tbl.df.columns,
                          "letter should be one column in the stringindex")
        invalidInputError(tbl.size() == 3, "the StringIndex should have three rows")
        cast_tbl = tbl.cast("id", "long")
        invalidInputError(isinstance(cast_tbl, StringIndex),
    ...
    def test_encode_string_from_dict(self):
        ...
        invalidInputError('name' in tbl.df.columns, "name should be still in the columns")
        invalidInputError('num' in tbl.df.columns, "num should be still in the columns")
        invalidInputError(tbl.df.where(tbl.df.age == 14).select("name").collect()[0]["name"] == 1,
                          "the first row of name should be 1")
        invalidInputError(tbl.df.where(tbl.df.height == 10).select("num").collect()[0]["num"] == 2,
    ...
    def test_write_csv(self):
        ...
        tbl.write_csv(directory, mode="overwrite", header=True, num_partitions=1,
                      delimiter="\t")
        invalidInputError(os.path.exists("write.csv"), "files not write")
        result = FeatureTable(spark.read.csv(directory, header=True, sep="\t"))
        invalidInputError(isinstance(result, FeatureTable), "type error")
        invalidInputError(result.size() == 3, "the size of result should be 3")
        invalidInputError(result.filter("age == 23").size() == 1, "wrong age")
        invalidInputError(result.filter("name == 'jack'").size() == 1, "wrong name")
        invalidInputError(result.filter("name == 'alice'").size() == 1, "wrong name")
    ...
    def test_concat(self):
        ...
        invalidInputError(tbl.size() == 2, "size error")
        tbl = tbl1.concat(tbl1, distinct=True)
        invalidInputError(tbl.size() == 1, "size error")
        tbl = tbl1.concat(tbl2)
        invalidInputError(tbl.filter("name == 'jack'").size() == 1, "size error")
        invalidInputError(tbl.filter("name == 'alice'").size() == 1, "size error")
        tbl = tbl1.concat(tbl3, mode="inner")
        invalidInputError(tbl.df.schema.names == ["name", "id"], "size error")
        tbl = tbl1.concat(tbl3, mode="outer")
        invalidInputError(tbl.df.schema.names == ["name", "id", "weight"], "size error")
        invalidInputError(tbl.fillna(0, "weight").filter("weight == 0").size() == 1,
                          "size error")
        tbl = tbl1.concat([tbl1, tbl2, tbl3])
        invalidInputError(tbl.size() == 4, "size error")
        invalidInputError(tbl.distinct().size() == 3, "size error")
        tbl = tbl1.concat([tbl1, tbl2, tbl3], distinct=True)
        invalidInputError(tbl.size() == 3, "size error")
    ...
    def test_drop_duplicates(self):
        ...
        tbl2.df.show()
        invalidInputError(tbl2.size() == 3, "size error")
        invalidInputError(tbl2.df.filter((tbl2.df.name == 'jack') & (tbl2.df.grade == 'a'))
                          .select("number").collect()[0]["number"] == 1, "size error")
        invalidInputError(tbl2.df.filter((tbl2.df.name == 'jack') & (tbl2.df.grade == 'b'))
                          .select("number").collect()[0]["number"] == 2, "size error")
        invalidInputError(tbl2.df.filter((tbl2.df.name == 'amy') & (tbl2.df.grade == 'a'))
                          .select("number").collect()[0]["number"] == 2, "size error")
        tbl3 = tbl.drop_duplicates(subset=['name', 'grade'], sort_cols='number', keep='max')
        tbl3.df.show()
        invalidInputError(tbl3.size() == 3, "size error")
        invalidInputError(tbl3.df.filter((tbl2.df.name == 'jack') & (tbl2.df.grade == 'a'))
                          .select("number").collect()[0]["number"] == 3, "size error")
        invalidInputError(tbl3.df.filter((tbl2.df.name == 'jack') & (tbl2.df.grade == 'b'))
                          .select("number").collect()[0]["number"] == 2, "size error")
        invalidInputError(tbl3.df.filter((tbl2.df.name == 'amy') & (tbl2.df.grade == 'a'))
                          .select("number").collect()[0]["number"] == 5, "size error")
        tbl4 = tbl.drop_duplicates(subset=None, sort_cols='number', keep='max')
        tbl4.df.show()
        invalidInputError(tbl4.size() == 6, "size error")
        tbl5 = tbl.drop_duplicates(subset=['name', 'grade'], sort_cols=None, keep='max')
        tbl5.df.show()
        invalidInputError(tbl5.size() == 3, "size error")
        tbl6 = tbl.drop_duplicates(subset=['name'], sort_cols=["grade", "number"], keep='max')
        invalidInputError(tbl6.size() == 2, "size error")
        tbl6.df.show()
        invalidInputError(tbl6.df.filter((tbl6.df.name == 'jack') & (tbl6.df.grade == 'b')
                                         & (tbl6.df.number == 2))
                          .select("number").collect()[0]["number"] == 2, "size error")
        invalidInputError(tbl6.df.filter((tbl6.df.name == 'amy') & (tbl6.df.grade == 'a')
                                         & (tbl6.df.number == 5))
                          .select("number").collect()[0]["number"] == 5, "size error")
    ...
    def test_join(self):
        ...
        invalidInputError("name_l" in tbl.df.schema.names, "col error")
        invalidInputError("id" in tbl.df.schema.names, "col error")
        invalidInputError("name_r" in tbl.df.schema.names, "col error")
    ...
    def test_cut_bins(self):
        ...
        invalidInputError("age_bucket" in new_tbl.columns, "col error")
        invalidInputError("ages" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("age_bucket").rdd.flatMap(lambda x: x).collect() ==
                          ["adult", "adult", "minor", "senior", "adult",
                           "infant", "adult", "adult", "adult"], "col error")
        # test out_col equal to input column
        new_tbl = tbl.cut_bins(bins=splits, columns="ages", labels=labels,
                               out_cols="ages", drop=True)
        invalidInputError("ages" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("ages").rdd.flatMap(lambda x: x).collect() ==
                          ["adult", "adult", "minor", "senior", "adult",
                           "infant", "adult", "adult", "adult"], "col error")
        # test name not defined
        new_tbl = tbl.cut_bins(bins=splits, columns="ages", labels=labels, drop=True)
        invalidInputError("ages_bin" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("ages_bin").rdd.flatMap(lambda x: x).collect() ==
                          ["adult", "adult", "minor", "senior", "adult",
                           "infant", "adult", "adult", "adult"], "col error")
        # test integer bins
        ...
        invalidInputError("ages_bin" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("ages_bin").rdd.flatMap(lambda x: x).collect() ==
                          ["minor", "adult", "minor", "senior", "adult",
                           "minor", "minor", "adult", "adult"], "col error")
        # test label is None
        new_tbl = tbl.cut_bins(bins=4, columns="ages", drop=True)
        invalidInputError("ages_bin" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("ages_bin").rdd.flatMap(lambda x: x).collect()
                          == [2, 3, 1, 5, 4, 1, 2, 3, 3], "col error")
        ...
        invalidInputError("ages_bin" in new_tbl.columns, "col error")
        invalidInputError("ages" in new_tbl.columns, "col error")
        invalidInputError("number_bin" in new_tbl.columns, "col error")
        invalidInputError(new_tbl.df.select("ages_bin").rdd.flatMap(lambda x: x).collect() ==
                          ["adult", "adult", "minor", "senior", "adult",
                           "infant", "adult", "adult", "adult"], "col error")
    ...
    def test_columns(self):
        ...
        invalidInputError(isinstance(col_names, list),
                          "col_names should be a list of strings")
        invalidInputError(col_names == ["col_1", "col_2", "col_3", "col_4", "col_5"],
    ...
    def test_get_stats(self):
        ...
        invalidInputError(len(statistics) == 2,
                          "the dict should contain two statistics")
        invalidInputError(statistics["age"] == 14,
                          "the min value of age is not correct")
        invalidInputError(statistics["height"] == 8.5,
                          "the min value of height is not correct")
        columns = ["age", "height"]
        # test dict
        statistics = tbl.get_stats(columns, {"age": "max", "height": "avg"})
        invalidInputError(len(statistics) == 2, "the dict should contain two statistics")
        invalidInputError(statistics["age"] == 25, "the max value of age is not correct")
        invalidInputError(statistics["height"] == 9.4, "the avg value of height is not correct")
        # test list
        statistics = tbl.get_stats(columns, ["min", "max"])
        invalidInputError(len(statistics) == 2, "the dict should contain two statistics")
        invalidInputError(statistics["age"][0] == 14, "the min value of age is not correct")
        invalidInputError(statistics["age"][1] == 25, "the max value of age is not correct")
        invalidInputError(statistics["height"][0] == 8.5, "the min value of height is not correct")
        invalidInputError(statistics["height"][1] == 10.0, "the max value of height is not correct")
        # test dict of list
        ...
        invalidInputError(statistics["age"][1] == 25, "the max value of age is not correct")
        invalidInputError(statistics["height"][0] == 8.5, "the min value of height is not correct")
        invalidInputError(statistics["height"][1] == 9.4, "the max value of height is not correct")
        statistics = tbl.get_stats(None, "min")
        invalidInputError(len(statistics) == 2, "the dict should contain two statistics")
        invalidInputError(statistics["age"] == 14, "the min value of age is not correct")
        invalidInputError(statistics["height"] == 8.5, "the min value of height is not correct")
    ...
    def test_min(self):
        ...
        invalidInputError(min_result.to_list("min") == [14, 8.5],
    ...
    def test_max(self):
        ...
        invalidInputError(min_result.to_list("max") == [25, 10.0],
    ...
    def test_to_list(self):
        ...
        invalidInputError(list1 == ["jack", "alice", "rose"],
                          "the result of name is not correct")
        invalidInputError(list2 == ["123", "34", "25344"],
                          "the result of num is not correct")
        invalidInputError(list3 == [14, 25, 23], "the result of age is not correct")
        invalidInputError(list4 == [8.5, 9.6, 10.0],
                          "the result of height is not correct")
        invalidInputError(list5 == [[0, 0], [1, 1], [2, 2]],
                          "the result of array is not correct")
    ...
    def test_to_dict(self):
        ...
        print(dictionary)
        invalidInputError(dictionary["name"] == ['jack', 'alice', 'rose'], "value error")
    ...
    def test_to_pandas(self):
        ...
        tbl = FeatureTable(spark.createDataFrame(data, schema))
        pddf = tbl.to_pandas()
        invalidInputError(pddf["name"].values.tolist() == ['jack', 'alice', 'rose'], "value error")
    ...
    def test_sort(self):
        ...
        invalidInputError(tbl.select("Name").to_list("Name") == ["nick", "juli", "tom"],
                          "col error")
        tbl = tbl.sort("Name")
        invalidInputError(tbl.select("Name").to_list("Name") == ["juli", "nick", "tom"],
                          "col error")
    ...
    def test_add(self):
        ...
        invalidInputError(len(new_list) == 3, "new_tbl should have 3 rows")
        invalidInputError(new_list[0]['age'] == 15.5, "the age of jack should increase 1.5")
        invalidInputError(new_list[0]['height'] == 10, "the height of jack should increase 1.5")
        invalidInputError(new_list[1]['age'] == 26.5, "the age of alice should increase 1.5")
        invalidInputError(new_list[1]['height'] == 11.1, "the height of alice should increase 1.5")
        invalidInputError(new_list[2]['age'] == 24.5, "the age of rose should increase 1.5")
        invalidInputError(new_list[2]['height'] == 11.5, "the height of rose should increase 1.5")
        new_tbl = tbl.add(columns, -1)
        new_list = new_tbl.df.take(3)
        invalidInputError(len(new_list) == 3, "new_tbl should have 3 rows")
        invalidInputError(new_list[0]['age'] == 13, "the age of jack should decrease 1")
        invalidInputError(new_list[0]['height'] == 7.5, "the height of jack should decrease 1")
        invalidInputError(new_list[1]['age'] == 24, "the age of alice should decrease 1")
        invalidInputError(new_list[1]['height'] == 8.6, "the height of alice should decrease 1")
        invalidInputError(new_list[2]['age'] == 22, "the age of rose should decrease 1")
        invalidInputError(new_list[2]['height'] == 9.0, "the height of rose should decrease 1")
    ...
    def test_group_by(self):
        ...
        invalidInputError(groupby_tbl1.df.filter("col_4 == 'a' and sum(col_1) == 3").count() == 1,
                          "the sum of col_1 with col_4 = 'a' should be 3")
        invalidInputError(groupby_tbl1.df.filter("col_4 == 'b' and `count(col_1)` == 5")
                          .count() == 1, "the count of col_1 with col_4 = 'b' should be 5")

        groupby_tbl2 = feature_tbl.group_by(agg={"target": "avg", "col_2": "last"})
        invalidInputError(groupby_tbl2.df.collect()[0]["avg(target)"] == 0.9,
                          "the mean of target should be 0.9")

        groupby_tbl3 = feature_tbl.group_by("col_5", agg=["max", "min"], join=True)
        invalidInputError(len(groupby_tbl3.df.columns) == len(feature_tbl.df.columns) + 10,
                          "groupby_tbl3 should have (#df.columns - #columns)*len(agg)=10 "
                          "more columns")
        invalidInputError(groupby_tbl3.df.filter("col_5 == 'cc' and `max(col_2)` == 9").count() ==
                          feature_tbl.df.filter("col_5 == 'cc'").count(),
                          "max of col_2 should 9 for all col_5 = 'cc' in groupby_tbl3")
        invalidInputError(groupby_tbl3.df.filter("col_5 == 'aa' and `min(col_3)` == 1.0").count() ==
        ...
        invalidInputError(
            groupby_tbl4.df.filter("col_4 == 'b' and col_5 == 'dd' and `first(col_1)` == 0")
    ...
    def test_append_column(self):
        ...
        invalidInputError(tbl.select("z").size() == 4, "size error")
        invalidInputError(tbl.filter("z == 0").size() == 4, "size error")
        tbl = tbl.append_column("str", lit("a"))
        invalidInputError(tbl.select("str").size() == 4, "size error")
        invalidInputError(tbl.filter("str == 'a'").size() == 4, "size error")
        tbl = tbl.append_column("float", lit(1.2))
        invalidInputError(tbl.select("float").size() == 4, "size error")
        invalidInputError(tbl.filter("float == 1.2").size() == 4, "size error")
    ...
    def test_ordinal_shuffle(self):
        ...
        invalidInputError(rows == shuffled_rows, "row error")
    ...
    def test_read_csv(self):
        ...
        invalidInputError(feature_tbl.size() == 4, "size error")
        columns = feature_tbl.columns
        invalidInputError(columns == ["col1", "col2", "col3"], "col error")
        records = feature_tbl.df.collect()
        invalidInputError(isinstance(records[0][0], float), "type error")
        invalidInputError(isinstance(records[0][1], str) and isinstance(records[0][1], str),
                          "type error")
        ...
        feature_tbl2 = FeatureTable.read_csv(file_path2, names=["col1", "_col2", "col3"],
                                             dtype={"col1": "int"})
        invalidInputError(feature_tbl2.size() == 4, "size error")
        columns2 = feature_tbl2.columns
        invalidInputError(columns2 == ["col1", "_col2", "col3"], "col error")
        records2 = feature_tbl2.df.collect()
        invalidInputError(isinstance(records2[0][0], int), "size error")
        invalidInputError(isinstance(records2[0][1], str) and isinstance(records2[0][1], str),
                          "type error")
        feature_tbl3 = FeatureTable.read_csv(file_path, header=True, dtype=["int", "str", "str"])
        records3 = feature_tbl3.df.collect()
        invalidInputError(isinstance(records3[0][0], int), "type error")
        invalidInputError(isinstance(records3[0][1], str) and isinstance(records3[0][1], str),
                          "type error")

    def test_read_text(self):
        file_path = os.path.join(self.resource_path, "data.csv")
        feature_tbl = FeatureTable.read_text(file_path, col_name="line")
        invalidInputError(feature_tbl.size() == 5, "size error")
        columns = feature_tbl.columns
        invalidInputError(columns == ["line"], "col error")
        rows = feature_tbl.df.collect()
        # when read as text, header will be treated as the first row
        invalidInputError(rows[0][0] == "col1,col2,col3", "col error")
        invalidInputError(rows[3][0] == "4.0,x,aaa", "col error")

    def test_category_encode_and_one_hot_encode(self):
        file_path = os.path.join(self.resource_path, "data.csv")
        feature_tbl = FeatureTable.read_csv(file_path, header=True)
        feature_tbl, indices = feature_tbl.category_encode(columns=["col2", "col3"])
        invalidInputError(isinstance(indices, list) and len(indices) == 2, "size error")
        invalidInputError(isinstance(indices[0], StringIndex) and
                          isinstance(indices[1], StringIndex), "type error")
        invalidInputError(indices[0].size() == 3 and indices[1].size() == 4, "size error")
        dict1 = indices[0].to_dict()
        dict2 = indices[1].to_dict()
        records = feature_tbl.df.collect()
        invalidInputError(records[0][1] == dict1["x"] and records[0][2] == dict2["abc"],
                          "result error")
        invalidInputError(records[3][1] == dict1["z"] and records[2][2] == dict2["aaa"],
                          "result error")
        feature_tbl = feature_tbl.one_hot_encode(columns=["col2", "col3"], prefix=["o1", "o2"])
        feature_tbl.show()
        columns = feature_tbl.columns
        invalidInputError(columns == ["col1", "o1_0", "o1_1", "o1_2", "o1_3", "o2_0",
                                      "o2_1", "o2_2", "o2_3", "o2_4"], "col error")
        records = feature_tbl.df.collect()
        record = records[0]
        value1 = dict1["x"]
        value2 = dict2["abc"]
    ...
    def test_split(self):
        ...
        size1 = tbl1.size()
        size2 = tbl2.size()
        invalidInputError(size1 + size2 == total_size, "size error")
    ...
    def test_target_encode(self):
        ...
        invalidInputError(len(target_list1) == 1,
                          "len(target_list1) = len(cat_cols) of target_encode")
        target_code1 = target_list1[0]
        invalidInputError(isinstance(target_code1, TargetCode),
                          "target_list1 should be list of TargetCode")
        invalidInputError(target_code1.df.filter("col_4 == 'a'").collect()[0]["col_4_te_target"] ==
        ...
        target_tbl2, target_list2 = feature_tbl.target_encode(
            ...
            drop_fold=False,
            out_cols=out_cols)
        invalidInputError(fold_col in target_tbl2.df.columns,
                          "fold_col should be in target_tbl2")
        invalidInputError(len(target_list2) == len(cat_cols),
                          "len(target_list2) = len(cat_cols)")
        for i in range(len(cat_cols)):
            invalidInputError(target_list2[i].cat_col == cat_cols[i],
                              "each element in target_list2 should "
                              "correspond to the element in cat_cols")
            for out_col in out_cols[i]:
                invalidInputError(out_col in target_list2[i].df.columns,
                                  "every out_cols should be one of "
                                  "the columns in returned TargetCode")
                invalidInputError(target_mean[target_list2[i].out_target_mean[out_col][0]] ==
                                  target_list2[i].out_target_mean[out_col][1],
                                  "the global mean in TargetCode should be the same as the "
                                  "assigned mean in target_mean")

        target_tbl3, target_list3 = feature_tbl.target_encode([["col_4", "col_5"]], "target",
                                                              kfold=2, drop_cat=False)
        invalidInputError(len(target_tbl3.columns) == len(feature_tbl.columns) + 1,
    ...
    def test_encode_target(self):
        ...
        invalidInputError(target_tbl1.df.filter("col_5_te_col_1 == 1").count() ==
                          feature_tbl.df.filter("col_5 == 'aa'").count(),
                          "the row with col_5 = 'aa' be encoded as "
                          "col_5_te_col_1 = 1 in target_tbl1")
        invalidInputError(target_tbl1.df.filter("col_3 == 8.0 and col_4 == 'd'")
        ...
        invalidInputError("col_4" in target_tbl3.df.columns,
                          "col_4 should exist in target_tbl2 since drop_cat is False")
        invalidInputError("col_4_te_target" in target_tbl3.df.columns,
                          "col_4_te_target should exist in target_tbl2 as encoded column")
        invalidInputError("col_4_te_col_3" not in target_tbl3.df.columns,
                          "col_4_te_col_3 should not exist in target_tbl2 since "
                          "col_3 is not in target_cols")
    ...
    def test_difference_lag(self):
        ...
        invalidInputError(diff_tbl1.df.filter("col_1_diff_lag_col_1_1 == 1").count() == 5 and
                          diff_tbl1.df.filter("col_1_diff_lag_col_1_1 == 0").count() == 13 and
                          diff_tbl1.df.filter("col_1_diff_lag_col_1_1 is null").count() == 2,
                          "col_1 has 6 different values and 1 null, so after sorted by col_1, "
                          "there should be 5 rows with (lag of col_1) = 1, 2 rows with "
                          "(lag of col_1) = null, and other rows with (lag of col_1) = 0")

        diff_tbl2 = feature_tbl.difference_lag(
            ["col_1", "col_2"], ["col_3"], shifts=[1, -1],
            partition_cols=["col_5"], out_cols=[["c1p1", "c1m1"], ["c2p1", "c2m1"]])
        invalidInputError(diff_tbl2.df.filter("col_3 == 8.0 and col_5 == 'cc'")
                          .filter("c1p1 == 1 and c1m1 == 2 and c2p1 == -1 and c2m1 == -4")
                          .count() == 1,
                          "the row with col_3 = 8.0 and col_5 = 'cc' should have c1p1 = 1, "
        ...
        invalidInputError(diff_tbl3.df.filter("c1m1 == 2").count() ==
        ...
        invalidInputError(diff_tbl4.df.filter("c1p1 == -1").count() ==
    ...
    def test_cache(self):
        ...
        feature_tbl = FeatureTable.read_parquet(file_path)
        feature_tbl.cache()
        invalidInputError(feature_tbl.df.is_cached, "Cache table should be cached")
        feature_tbl.uncache()
        invalidInputError(not feature_tbl.df.is_cached, "Uncache table should be uncached")

    def test_sample_listwise(self):
        spark = OrcaContext.get_spark_session()
        data = [("a", [1, 2, 3], ["1", "2", "3"], [[1], [2], [3]]),
                ...
                ("d", [1, 2], ["1", "2", "3"], [[1], [2], [3], [4]])]
        schema = StructType([StructField("name", StringType(), True),
                             StructField("int_arr", ArrayType(IntegerType()), True),
        ...
        with self.assertRaises(Exception) as context:
            tbl.sample_listwise(["aa"], num_sampled_list=1, num_sampled_item=1)
        self.assertTrue("does not exist in this FeatureTable." in str(context.exception))
        with self.assertRaises(Exception) as context:
            tbl2 = tbl.sample_listwise(["int_arr", "str_arr"], num_sampled_list=3,
                                       num_sampled_item=2)
            tbl2.show()
        self.assertTrue("Each row of the FeatureTable should" in str(context.exception))
        tbl = tbl.filter("name != 'd'")
        tbl3 = tbl.sample_listwise(["int_arr", "str_arr"], num_sampled_list=3,
                                   num_sampled_item=2)
        invalidInputError(tbl3.size() == 6, "size error")
        invalidInputError(tbl3.filter("size(int_arr) = 2").size() == 6, "size error")
        arr = tbl3.df.collect()
        for r in arr:
            invalidInputError(r["int_arr"] == [int(elem) for elem in r["str_arr"]], "value error")

        tbl4 = tbl.sample_listwise(["int_arr", "str_arr", "int_arr_arr"], num_sampled_list=4,
                                   num_sampled_item=2)
        invalidInputError(tbl4.size() == 8, "size error")
        invalidInputError(tbl4.filter("size(int_arr_arr) = 2").size() == 8, "size error")
        tbl5 = tbl.sample_listwise(["int_arr", "str_arr"], num_sampled_list=2,
                                   num_sampled_item=2, replace=False)
        invalidInputError(tbl5.size() == 4, "size error")
        invalidInputError(tbl5.filter("size(int_arr) = 2").size() == 0, "size error")
